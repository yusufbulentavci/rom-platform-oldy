...
class AuthImpl extends Auth {
	...
	AuthImpl() {
		...
		// Default to use the bundled oauthWindow.html
				Location.getProtocol() + "//" + Location.getHostName() + "/_public/oauthWindow.html");
		// "http://home.mlos.net/_public/login/oauthWindow.html");
		register();
	}
	 ...
	 */
	// TODO(jasonhall): This will not result in CookieStoreImpl being compiled
	// out
	// for browsers that support localStorage, and vice versa? If not, this
	// should
	...
	private native void register() /*-{
		...
		$wnd.oauth2.__doLogin = $entry(function(hash) {
			self.@com.bilgidoku.rom.site.kamu.login.client.oauth.AuthImpl::finish(Ljava/lang/String;)(hash);
		});
	}-*/;
...
//		return $wnd.open(url, 'popupWindow', 'width=' + width + ',height='
//				+ height);
//	}-*/;
