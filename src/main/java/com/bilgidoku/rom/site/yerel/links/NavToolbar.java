...
public class NavToolbar extends NavToolbarBase {
	private final NavLinks nav;

	public NavToolbar(NavLinks wa) {
		this.nav = wa;
		Widget[] btns = { btnNewContainer, btnReload, btnNew, ClientUtil.getSeperator(), btnEdit, btnDelete, btnCopyLink };
	...
	public void deleteItem() {
		...
		if (data.isContainer())
			LinksDao.extinct(data.getUri(), new StringResponse() {
			...
			LinksDao.destroy(data.getUri(), new StringResponse() {
				public void ready(String value) {
					nav.getTree().getSelectedItem().remove();
					Ctrl.closeTab(data.getUri());
				}
	...
	public void newItem() {
		...
		final NewItemDlg dlg = new NewItemDlg(cont.getUriPrefix(), Ctrl.trans.newLink(), false, null, cont.getUri());
		dlg.addCloseHandler(new CloseHandler<PopupPanel>() {
			@Override
			public void onClose(CloseEvent<PopupPanel> event) {
				if (dlg.getUri() != null && !dlg.getUri().isEmpty()) {
					LinksDao.neww(Ctrl.infoLang(), dlg.getNamed(), dlg.getUri(), dlg.getDelegated(),
							new StringResponse() {
						public void ready(String value) {
	...
	public void newContainer() {
		final NewItemDlg dlg = new NewItemDlg(nav.getHolderUriPrefix(), Ctrl.trans.newContainer(), true,
				Data.LINKS_ROOT + "/", nav.getHolderUri());
		dlg.addCloseHandler(new CloseHandler<PopupPanel>() {
			@Override
			public void onClose(CloseEvent<PopupPanel> event) {
				String title = dlg.getNamed();
				if (title != null && !title.isEmpty()) {
					LinksDao.breed(dlg.getUri(), Data.WRITING_PUBLIC_MASK, dlg.getDelegated(),
	...
	public void reloadContainer(TreeItem item) {
		...
		if (pd == null) {
			...
			if ((item.getChildCount() == 1 && item.getChild(0).getText().equals("!")) || reload) {
				...
				if (nav.listItems)
					LinksDao.list(Ctrl.infoLang(), "", pd.getUri(), new ContentsResponse() {
