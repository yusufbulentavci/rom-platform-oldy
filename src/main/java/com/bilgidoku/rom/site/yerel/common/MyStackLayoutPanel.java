 ...
 *
 * <p>
 * <h3>Example</h3> {@example com.google.gwt.examples.StackLayoutPanelExample}
 * </p>
 *
 ...
 * </pre>
 */
public class MyStackLayoutPanel extends ResizeComposite implements HasWidgets, ProvidesResize, IndexedPanel.ForIsWidget,
	 ...
	 */
	public MyStackLayoutPanel(Unit unit) {
		this.unit = unit;
	 ...
	 * @param header
	 *            the html to be shown on its header
	 * @param headerSize
	 ...
	 *            the child widget to be added
	 * @param header
	 *            the text to be shown on its header
	 * @param asHtml
	...
	public void animate(int duration, AnimationCallback callback) {
		...
		for (; i < layoutData.size(); ++i) {
			...
			layoutPanel.setWidgetLeftWidth(data.header, left, unit, data.headerSize, unit);

			left += data.headerSize;

			layoutPanel.setWidgetLeftWidth(data.widget, left, unit, 0, unit);
		...
		for (int j = layoutData.size() - 1; j > i; --j) {
			...
			layoutPanel.setWidgetRightWidth(data.header, right, unit, data.headerSize, unit);
			layoutPanel.setWidgetRightWidth(data.widget, right, unit, 0, unit);
			right += data.headerSize;
		}

		LayoutData data = layoutData.get(selectedIndex);
		layoutPanel.setWidgetLeftRight(data.widget, left, unit, right, unit);
	 ...
	 *
	 * @param index
	 *            the index of the stack header to be retrieved
	 * @return the header widget
	 */
	public Widget getHeaderWidget(int index) {
		checkIndex(index);
		return layoutData.get(index).header.asWidget();
	}

	/**
	 * Gets the widget in the stack header associated with the given child
	 * widget.
	 *
	 * @param child
	 *            the child whose stack header is to be retrieved
	 * @return the header widget
	 */
	 ...
	 * @param html
	 *            the safe html to be shown on its header
	 * @param headerSize
	 *            the size of the header widget
	 * @param beforeIndex
	 ...
	 *            the widget to be added
	 * @param header
	 *            the widget to be placed in the associated header
	 * @param headerSize
	 *            the size of the header widget
	 * @param beforeIndex
	 *            the index before which it will be inserted
	...
	public Iterator<Widget> iterator() {
		return new Iterator<Widget>() {
			...
			public void remove() {
				...
				MyStackLayoutPanel.this.remove(layoutData.get(last).widget);
				i = last;
				last = -1;
			}
		};
	 ...
	 *
	 * @param index
	 *            the index of the header whose HTML is to be set
	 * @param html
	...
	public void setHeaderHTML(int index, String html) {
		...
		Widget headerWidget = data.header.asWidget();
	 ...
	 *
	 * @param index
	 *            the index of the header whose text is to be set
	 * @param text
