package com.bilgidoku.rom.site.yerel.htmlsanitize;
...
public class HtmlSanitizerUtil {
	...
	private static final Set<String> TAG_WHITELIST = new HashSet<String>(Arrays.asList("a", "abbr", "acronym",
			"address", "area", "b", "big", "blockquote", "br", "button", "caption", "center", "cite", "code", "col",
			"colgroup", "dd", "del", "dfn", "dir", "div", "dl", "dt", "em", "fieldset", "font", "form", "h1", "h2",
			"h3", "h4", "h5", "h6", "hr", "i", "img", "input", "ins", "kbd", "label", "legend", "li", "map", "menu",
			"ol", "optgroup", "option", "p", "pre", "q", "s", "samp", "select", "small", "span", "strike", "strong",
			"style", "sub", "sup", "table", "tbody", "td", "textarea", "tfoot", "th", "thead", "tr", "tt", "u", "ul",
			"var"));
	 ...
	 * 
	 * <p>
	 * The input string is processed as described above. The result of
	 * sanitizing the string is guaranteed to be safe to use (with respect to
	 * XSS vulnerabilities) in HTML contexts, and is returned as an instance of
	 * the {@link SafeHtml} type.
	 * 
	 * @param html
	 *            the input String
	 ...
	 * approach is as follows: We split the string at each occurence of '<'.
	...
	private static String simpleSanitize(String text) {
		...
		for (String segment : text.split("<", -1)) {
			if (firstSegment) {
				 ...
				 * the input string starts with a tag, we will get an empty
			...
			/*
			 * determine if the current segment is the start of an
			 * attribute-free tag or end-tag in our whitelist
			 */
			...
			if (tagEnd > 0) {
				...
				if (TAG_WHITELIST.contains(exist)) {
					...
					Sistem.outln("not in whitelist tag=" + tag);
				}
		...
		// get rid weird style stuff - style="&#97;&#110;&#121;&#58;..." or
		// style="&#x61;&#x6e;&#x79;&#x3a;..."
	 ...
	 * construction of SafeHtml objects that are not stable in the sense that
	 * for a {@code SafeHtml s} it may not be true that {@code s.asString()}
	 * equals {@code SimpleHtmlSanitizer.sanitizeHtml(s.asString()).asString()}.
	 * While this is not currently an issue, it might become one and result in
