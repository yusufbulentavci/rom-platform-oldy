...
public class WindowBox extends DialogBox implements HasOpenHandlers<WindowBox> {
	 ...
	 *            cursor on
	 */
	protected static Cursor getCursor(int dm) {
		Cursor cursor;
	...
	protected void updateCursor(int dragMode) {
		if (this.resizable) {
			Cursor dm = getCursor(dragMode);
			RomEntryPoint.one.updateCursor(dm, this.getElement());

//			com.google.gwt.dom.client.Element top = this.getCellElement(0, 1);
//			RomEntryPoint.one.updateCursor(dm, top);

//			top = Element.as(top.getFirstChild());
//			if (top != null)
//				getCursor(dragMode, top.get);
		}
	}
	...
	protected void beginDragging(MouseDownEvent event) {
		...
		if (this.resizable && dm >= 0) {
			...
			RomEntryPoint.one.setCursor(getCursor(dm));
	...
	protected void endDragging(MouseUpEvent event) {
		if (this.dragMode >= 0 && this.resizable) {
			...
			this.dragMode = -1;
			this.updateCursor(this.dragMode);
			RomEntryPoint.one.setCursor(Cursor.AUTO);
		} else {
