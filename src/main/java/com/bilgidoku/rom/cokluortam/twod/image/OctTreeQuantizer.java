...
package com.bilgidoku.rom.cokluortam.twod.image;
	...
	class OctTreeNode {
		...
		public void list(PrintStream s, int level) {
			for (int i = 0; i < level; i++)
				System.out.print(' ');
//			if (count == 0)
//				syso(index+": count="+count);
//			else
//				syso(index+": count="+count+" red="+(totalRed/count)+" green="+(totalGreen/count)+" blue="+(totalBlue/count));
			for (int i = 0; i < 8; i++)
				if (leaf[i] != null)
...
//		syso("insertColor="+Integer.toHexString(rgb));
		for (int level = 0; level <= MAX_LEVEL; level++) {
			OctTreeNode child;
			int bit = 0x80 >> level;
...
//		syso("Unable to reduce the OctTree");
	}
