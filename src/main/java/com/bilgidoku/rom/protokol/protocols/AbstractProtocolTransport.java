 ...
 * already takes care of all the complex stuff when handling {@link Response}'s.
 * 
 * 
 * 
 */
...
//		com.bilgidoku.rom.min.Sistem.outln("<<<<" + response.getLines());
	 ...
	 * remote client. This method is aware of {@link FutureResponse} and makes
	 * sure the {@link Response}'s are written in the correct order
	 * 
		...
		if (response != null) {
			...
			if (response instanceof StartTlsResponse) {
				if (isStartTLSSupported()) {
					...
					// StartTls is not supported by this transport, so throw a
					// exception
					throw new UnsupportedOperationException(
							"StartTls is not supported by this ProtocolTransport implementation");
				}
			}
	 ...
	 *            the bytes to write
	 * @param session
	 *            the {@link ProtocolSession} for the write request
	 * @param startTLS
	 *            true if startTLS should be started after the bytes were
	 ...
	 * 
	 * @param in
	 *            the {@link InputStream} which should be written back to the
	 *            client
	 * @param session
	 *            the {@link ProtocolSession} for the write request
	 * @param startTLS
	 *            true if startTLS should be started after the
	 *            {@link InputStream} was written to the client
	 */
