...
import com.bilgidoku.rom.protokol.protocols.BaseRequest;
import com.bilgidoku.rom.protokol.protocols.CommandLog;
...
import com.bilgidoku.rom.protokol.protocols.future.FutureResponse.ResponseListener;
import com.bilgidoku.rom.protokol.protocols.future.FutureResponseImpl;
...
public class CommandDispatcher<Session extends ProtocolSession> implements ExtensibleHandler, LineHandler<Session> {
	private final static MC mc = new MC(CommandDispatcher.class);
	/**
...
//	private static final Astate wireCount = mc.c("command-dispatch-wire-count");
...
//		wireCount.more();
	...
	private static final Astate unabletoparse = mc.c("unable-to-parse-request");
	...
	public Response onLine(final Session session, ByteBuffer line) {
		...
		try {
			...
			//			com.bilgidoku.rom.min.Sistem.outln(">>" + request.toString());
			if (request == null) {
				return null;
			}
			return dispatchCommandHandlers(session, request);
		} catch (Exception e) {
			unabletoparse.more(e);
	 ...
	 * @throws KnownError 
	 */
	protected Response dispatchCommandHandlers(Session session, Request request) throws KnownError {
		List<CommandHandler<Session>> commandHandlers = getCommandHandlers(request.getCommand(), session);
		if(commandHandlers==null)
			return null;
		// fetch the command handlers registered to the command
		Iterator<CommandHandler<Session>> handlers=null;


			handlers = commandHandlers.iterator();

			while (handlers.hasNext()) {
				final long start = System.currentTimeMillis();
				CommandHandler<Session> cHandler = handlers.next();
				//            com.bilgidoku.rom.min.Sistem.outln("CommandHandler:"+cHandler.getClass());
				CommandLog cl = session.addCommandLog(request.getCommand(), request.getArgument());
				Response response = null;
				try {
					response = cHandler.onCommand(session, request);
					if (response != null) {
						long executionTime = System.currentTimeMillis() - start;

						// now process the result handlers
						response = executeResultHandlers(session, response, executionTime, cHandler,
								rHandlers.iterator());
						if (response != null) {
							return response;
						}
					...
					if (response != null) {
						cl.setResponse(response.getRetCode(), response.isSuccess(), response.isEndSession(),
