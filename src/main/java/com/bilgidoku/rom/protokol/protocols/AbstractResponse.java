...
public abstract class AbstractResponse implements Response{
    ...
    public Response immutable() {
        return new Response() {
			...
			public boolean isSuccess() {
				return AbstractResponse.this.isSuccess();
			}

			@Override
			public boolean isImportant() {
				return AbstractResponse.this.isImportant();
			}

			@Override
			public JSONObject toReport() throws JSONException {
				return AbstractResponse.this.toReport();
			}
        };
    }
	...
	public JSONObject toReport() throws JSONException {
		...
		jo.put("rc", this.retCode);
		if(endSession)
			jo.put("es", true);
		if(this.lines.size()>0){

			if(this.lines.size()==1){
				jo.put("desc", this.lines.get(0).toString());
			}else{
				StringBuilder sb=new StringBuilder();
				for (CharSequence it : this.lines) {
					sb.append(it.toString());
				}
				jo.put("desc", sb.toString());
			}
		}
		return jo;
	}
