...
public class ReceivedDataLineFilter extends AbstractAddHeadersFilter {
    ...
    protected Collection<Header> headers(SMTPSession session) {
        ...
        String heloMode = session.getHeloMode();
        String heloName = session.getCurHeloName();
        ...
        if (session.getRcptList().size() == 1) {
            ...
            headerLineBuffer = new StringBuilder();
            headerLineBuffer.append("for <").append(session.getRcptList().get(0).toString()).append(">;");
        } else {
