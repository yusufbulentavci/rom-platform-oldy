...
public abstract class SeparatingDataLineFilter implements DataLineFilter{
    ...
    /*
     * (non-Javadoc)
     * @see com.bilgidoku.rom.smtp.core.DataLineFilter#onLine(com.bilgidoku.rom.smtp.SMTPSession, java.nio.ByteBuffer, com.bilgidoku.rom.protokol.protocols.handler.LineHandler)
     */
    public final Response onLine(SMTPSession session, ByteBuffer line, LineHandler<SMTPSession> next) throws KnownError {
        if (session.getHeadersComplete() == null) {
            if (line.remaining() == 2 ) {
                if (line.get() == '\r' && line.get() == '\n') {
                    line.rewind();
                    Response response = onSeparatorLine(session, line, next);
                    session.setHeadersComplete(Boolean.TRUE);
                    return response;
                }
                line.rewind();
            }
     ...
     * 
     * This implementation just calls {@link LineHandler#onLine(com.bilgidoku.rom.protokol.protocols.ProtocolSession, ByteBuffer)} but subclasses should override it if needed.
     *
     * @param session
     * @param line
     * @param next
     ...
     * @throws KnownError 
     */
    protected Response onBodyLine(SMTPSession session, ByteBuffer line, LineHandler<SMTPSession> next) throws KnownError {
        return next.onLine(session, line);
    }
}
