...
public class Lists_change extends DbDaoBase{

	private final String callProto="select * from site.lists_change(?,?,?::text[])";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder content_idssqlType=new ArrayCoder(new StringCoder());
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("content_ids", null, null, false);
				content_idssqlType.setDbValue(db3, content_idssqlType.decode(val));
