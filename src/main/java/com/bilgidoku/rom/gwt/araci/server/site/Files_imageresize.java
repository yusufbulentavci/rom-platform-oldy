...
public class Files_imageresize extends DbDaoBase{

	private final String callProto="select * from site.files_imageresize(?,?,?,?)";

	private final BeforeHook beforeHook=getBeforeHook("com.bilgidoku.rom.web.db.image.resize");


		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder xsqlType=new IntegerCoder();
		TypeCoder ysqlType=new IntegerCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("x", null, null, false);
				xsqlType.setDbValue(db3, xsqlType.decode(val));

				val=request.getParam("y", null, null, false);
				ysqlType.setDbValue(db3, ysqlType.decode(val));
