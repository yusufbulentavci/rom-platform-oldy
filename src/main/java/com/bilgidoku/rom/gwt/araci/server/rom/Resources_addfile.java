...
public class Resources_addfile extends DbDaoBase{

	private final String callProto="select * from rom.resources_addfile(?,?,?)";

	private final BeforeHook beforeHook=getBeforeHook("com.bilgidoku.rom.web.db.files.dbfsadd");
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				{
					String fi=getFileItem(request, "fn");
					if(fi==null)
