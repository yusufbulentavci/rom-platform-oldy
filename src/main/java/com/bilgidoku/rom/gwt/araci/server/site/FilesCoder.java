...
public class FilesCoder extends
		TypeCoder<Files> {

	public FilesCoder(){
		super(true,new String[]{"ri","host_id","uri","container","html_file","modified_date","creation_date","delegated","ownercid","gid","relatedcids","mask","nesting","dbfs","weight","lexemes","rtags","aa","title","summary","tip","icon","medium_icon","large_icon","multilang_icon","sound","langcodes","viewy","metas"});
	}

	@Override
	public  Files decode(Object js) throws JSONException  {
		...
		c.metas=new ArrayCoder<String>(new StringCoder()).decode(json.get("metas"));
	...
	@Override
	public Object encode(Files obj) throws JSONException {
		...
		js.put("metas",new ArrayCoder<String>(new StringCoder()).encode(obj.metas));

		return js;
	}

	@Override
	public  Files[] createArray(int size) {
		return new  Files[size];
	}

	@Override
	public void setDbValue(DbSetGet db3, Files val) throws KnownError {
		if (val == null) {
		...
		new ArrayCoder<String>(new StringCoder()).setDbValue(db3,val.metas);

	}

	@Override
	public String toString(Files val) throws KnownError {
		...
		sb.append(',');sb.append(new ArrayCoder<String>(new StringCoder()).quoted(val.metas));
	...
	@Override
	protected Files inGetDbValue(DbSetGet db3) throws KnownError, JSONException {
		Files val=new Files();
		...
		val.metas=new ArrayCoder<String>(new StringCoder()).getDbValue(db3);

		return val;
	}

	public Files fromString(String str) throws JSONException{
		if(str==null)
			return null;
		...
		val.metas=new ArrayCoder<String>(new StringCoder()).fromString(ms[i++]);
