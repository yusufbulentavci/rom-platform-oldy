...
public class Cart_setshipaddr extends DbDaoBase{

	private final String callProto="select * from site.cart_setshipaddr(?,?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder shipaddrsqlType=new JsonCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("shipaddr", null, null, false);
				shipaddrsqlType.setDbValue(db3, shipaddrsqlType.decode(val));
