...
public class Cart_setcancelled extends DbDaoBase{

	private final String callProto="select * from site.cart_setcancelled(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder cancelledsqlType=new BooleanCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("cancelled", null, null, false);
				cancelledsqlType.setDbValue(db3, cancelledsqlType.decode(val));
