...
public class Info_setlogin extends DbDaoBase{

	private final String callProto="select * from site.info_setlogin(?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder loginsqlType=new BooleanCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("login", null, null, false);
				loginsqlType.setDbValue(db3, loginsqlType.decode(val));
