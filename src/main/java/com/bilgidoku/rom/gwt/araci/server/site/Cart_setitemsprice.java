...
public class Cart_setitemsprice extends DbDaoBase{

	private final String callProto="select * from site.cart_setitemsprice(?,?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder itemspricesqlType=new JsonCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("itemsprice", null, null, false);
				itemspricesqlType.setDbValue(db3, itemspricesqlType.decode(val));
