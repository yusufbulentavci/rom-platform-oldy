...
public class Exams_setquestions extends DbDaoBase{

	private final String callProto="select * from site.exams_setquestions(?,?,?::text[])";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder questionssqlType=new ArrayCoder(new StringCoder());
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("questions", null, null, false);
				questionssqlType.setDbValue(db3, questionssqlType.decode(val));
