...
public class Cart_add extends DbDaoBase{

	private final String callProto="select * from site.cart_add(?,?,?,?,?,?::rom.langs,?::rom.langs)";
		...
		TypeCoder stocksqlType=new StringCoder();
		TypeCoder diffsqlType=new IntegerCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("stock", null, null, false);
				stocksqlType.setDbValue(db3, stocksqlType.decode(val));

				val=request.getParam("diff", null, null, false);
				diffsqlType.setDbValue(db3, diffsqlType.decode(val));
