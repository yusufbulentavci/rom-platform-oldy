...
public class HouseholdsCoder extends
		TypeCoder<Households> {

	public HouseholdsCoder(){
		super(true,new String[]{"ri","host_id","uri","container","html_file","modified_date","creation_date","delegated","ownercid","gid","relatedcids","mask","nesting","dbfs","weight","lexemes","rtags","aa","title","summary","tip","icon","medium_icon","large_icon","multilang_icon","sound","langcodes","viewy","aktifmi","baslangic","bitis"});
	}

	@Override
	public  Households decode(Object js) throws JSONException  {
		JSONObject json = getObject(js);
		if(json==null)
			return null;
		 Households c=new Households();
		...
		c.aktifmi=new BooleanCoder().decode(json.get("aktifmi"));
		c.baslangic=new StringCoder().decode(json.get("baslangic"));
		c.bitis=new StringCoder().decode(json.get("bitis"));
	...
	@Override
	public Object encode(Households obj) throws JSONException {
		...
		js.put("aktifmi",new BooleanCoder().encode(obj.aktifmi));
		js.put("baslangic",new StringCoder().encode(obj.baslangic));
		js.put("bitis",new StringCoder().encode(obj.bitis));

		return js;
	}

	@Override
	public  Households[] createArray(int size) {
		return new  Households[size];
	}

	@Override
	public void setDbValue(DbSetGet db3, Households val) throws KnownError {
		if (val == null) {
		...
		new BooleanCoder().setDbValue(db3,val.aktifmi);
		new StringCoder().setDbValue(db3,val.baslangic);
		new StringCoder().setDbValue(db3,val.bitis);

	}

	@Override
	public String toString(Households val) throws KnownError {
		...
		sb.append(',');sb.append(new BooleanCoder().quoted(val.aktifmi));
		sb.append(',');sb.append(new StringCoder().quoted(val.baslangic));
		sb.append(',');sb.append(new StringCoder().quoted(val.bitis));
	...
	@Override
	protected Households inGetDbValue(DbSetGet db3) throws KnownError, JSONException {
		Households val=new Households();
		...
		val.aktifmi=new BooleanCoder().getDbValue(db3);
		val.baslangic=new StringCoder().getDbValue(db3);
		val.bitis=new StringCoder().getDbValue(db3);

		return val;
	}

	public Households fromString(String str) throws JSONException{
		if(str==null)
			return null;
		...
		val.aktifmi=new BooleanCoder().fromString(ms[i++]);
		val.baslangic=new StringCoder().fromString(ms[i++]);
		val.bitis=new StringCoder().fromString(ms[i++]);
