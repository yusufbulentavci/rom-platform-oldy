...
public class TariffmodelCoder extends
		TypeCoder<Tariffmodel> {

	public TariffmodelCoder(){
		super(true,new String[]{"ri","host_id","uri","container","html_file","modified_date","creation_date","delegated","ownercid","gid","relatedcids","mask","nesting","dbfs","weight","lexemes","rtags","aa","title","code","baseprice","vatpercentage","coefficient"});
	}

	@Override
	public  Tariffmodel decode(Object js) throws JSONException  {
		JSONObject json = getObject(js);
		if(json==null)
			return null;
		 Tariffmodel c=new Tariffmodel();
		...
		c.baseprice=new StringCoder().decode(json.get("baseprice"));
		c.vatpercentage=new IntegerCoder().decode(json.get("vatpercentage"));
		c.coefficient=new StringCoder().decode(json.get("coefficient"));
	...
	@Override
	public Object encode(Tariffmodel obj) throws JSONException {
		...
		js.put("baseprice",new StringCoder().encode(obj.baseprice));
		js.put("vatpercentage",new IntegerCoder().encode(obj.vatpercentage));
		js.put("coefficient",new StringCoder().encode(obj.coefficient));

		return js;
	}

	@Override
	public  Tariffmodel[] createArray(int size) {
		return new  Tariffmodel[size];
	}

	@Override
	public void setDbValue(DbSetGet db3, Tariffmodel val) throws KnownError {
		if (val == null) {
		...
		new StringCoder().setDbValue(db3,val.baseprice);
		new IntegerCoder().setDbValue(db3,val.vatpercentage);
		new StringCoder().setDbValue(db3,val.coefficient);

	}

	@Override
	public String toString(Tariffmodel val) throws KnownError {
		...
		sb.append(',');sb.append(new StringCoder().quoted(val.baseprice));
		sb.append(',');sb.append(new IntegerCoder().quoted(val.vatpercentage));
		sb.append(',');sb.append(new StringCoder().quoted(val.coefficient));
	...
	@Override
	protected Tariffmodel inGetDbValue(DbSetGet db3) throws KnownError, JSONException {
		Tariffmodel val=new Tariffmodel();
		...
		val.baseprice=new StringCoder().getDbValue(db3);
		val.vatpercentage=new IntegerCoder().getDbValue(db3);
		val.coefficient=new StringCoder().getDbValue(db3);

		return val;
	}

	public Tariffmodel fromString(String str) throws JSONException{
		if(str==null)
			return null;
		...
		val.baseprice=new StringCoder().fromString(ms[i++]);
		val.vatpercentage=new IntegerCoder().fromString(ms[i++]);
		val.coefficient=new StringCoder().fromString(ms[i++]);
