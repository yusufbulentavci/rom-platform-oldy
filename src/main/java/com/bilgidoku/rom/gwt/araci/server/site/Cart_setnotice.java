...
public class Cart_setnotice extends DbDaoBase{

	private final String callProto="select * from site.cart_setnotice(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder noticesqlType=new StringCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("notice", null, null, false);
				noticesqlType.setDbValue(db3, noticesqlType.decode(val));
