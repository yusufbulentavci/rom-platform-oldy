...
public class Exams_setrequirements extends DbDaoBase{

	private final String callProto="select * from site.exams_setrequirements(?,?,?::text[])";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder requirementssqlType=new ArrayCoder(new StringCoder());
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("requirements", null, null, false);
				requirementssqlType.setDbValue(db3, requirementssqlType.decode(val));
