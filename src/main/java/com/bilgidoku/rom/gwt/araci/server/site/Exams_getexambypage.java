...
public class Exams_getexambypage extends DbDaoBase{

	private final String callProto="select * from site.exams_getexambypage(?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder pagesqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("page", null, null, false);
				pagesqlType.setDbValue(db3, pagesqlType.decode(val));
