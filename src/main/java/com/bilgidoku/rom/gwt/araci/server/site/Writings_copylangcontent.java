...
public class Writings_copylangcontent extends DbDaoBase{

	private final String callProto="select * from site.writings_copylangcontent(?,?,?::rom.langs,?::rom.langs,?::rom.langs,?,?,?,?)";
		...
		TypeCoder fromlangsqlType=new StringCoder();
		TypeCoder spotsqlType=new BooleanCoder();
		TypeCoder bodysqlType=new BooleanCoder();
		TypeCoder headersqlType=new BooleanCoder();
		TypeCoder footersqlType=new BooleanCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("fromlang", null, null, false);
				fromlangsqlType.setDbValue(db3, fromlangsqlType.decode(val));

				val=request.getParam("spot", null, null, false);
				spotsqlType.setDbValue(db3, spotsqlType.decode(val));

				val=request.getParam("body", null, null, false);
				bodysqlType.setDbValue(db3, bodysqlType.decode(val));

				val=request.getParam("header", null, null, false);
				headersqlType.setDbValue(db3, headersqlType.decode(val));

				val=request.getParam("footer", null, null, false);
				footersqlType.setDbValue(db3, footersqlType.decode(val));
