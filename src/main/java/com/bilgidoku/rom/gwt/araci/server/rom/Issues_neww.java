...
public class Issues_neww extends DbDaoBase{

	private final String callProto="select * from rom.issues_new(?,?,?::rom.langs,?::rom.langs,?,?,?,?,?)";
		...
		TypeCoder descsqlType=new StringCoder();
		TypeCoder attachsqlType=new StringCoder();
		TypeCoder clssqlType=new StringCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"rom.issues.new", auditParams, request.getAuditParams(auditParams));
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("desc", null, null, false);
				descsqlType.setDbValue(db3, descsqlType.decode(val));

				{
					String fi=getFileItem(request, "attach");
					if(fi==null)
						db3.setNull(java.sql.Types.VARCHAR);
					else
						db3.setString(fi);
				...
				val=request.getParam("cls", null, null, false);
				clssqlType.setDbValue(db3, clssqlType.decode(val));
