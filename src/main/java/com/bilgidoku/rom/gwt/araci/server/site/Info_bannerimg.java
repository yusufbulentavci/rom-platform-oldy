...
public class Info_bannerimg extends DbDaoBase{

	private final String callProto="select * from site.info_bannerimg(?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder bannerimgsqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("bannerimg", null, null, false);
				bannerimgsqlType.setDbValue(db3, bannerimgsqlType.decode(val));
