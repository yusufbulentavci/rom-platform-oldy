...
public class Hits_list extends DbDaoBase{

	private final String callProto="select * from site.hits_list(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder sincesqlType=new LongCoder();
		TypeCoder timeendsqlType=new LongCoder();


	TypeCoder coder=new HitsCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("timeend", null, null, false);
				timeendsqlType.setDbValue(db3, timeendsqlType.decode(val));
