...
public class Dialogs_setcafe extends DbDaoBase{

	private final String callProto="select * from rom.dialogs_setcafe(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder cafesqlType=new BooleanCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("cafe", null, null, false);
				cafesqlType.setDbValue(db3, cafesqlType.decode(val));
