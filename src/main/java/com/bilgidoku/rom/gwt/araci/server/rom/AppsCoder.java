...
public class AppsCoder extends
		TypeCoder<Apps> {

	public AppsCoder(){
		super(true,new String[]{"ri","host_id","uri","container","html_file","modified_date","creation_date","delegated","ownercid","gid","relatedcids","mask","nesting","dbfs","weight","lexemes","rtags","aa","title","codes","cls","state"});
	}

	@Override
	public  Apps decode(Object js) throws JSONException  {
		...
		c.cls=new IntegerCoder().decode(json.get("cls"));
		c.state=new JsonCoder().decode(json.get("state"));
	...
	@Override
	public Object encode(Apps obj) throws JSONException {
		...
		js.put("cls",new IntegerCoder().encode(obj.cls));
		js.put("state",new JsonCoder().encode(obj.state));

		return js;
	}

	@Override
	public  Apps[] createArray(int size) {
		return new  Apps[size];
	}

	@Override
	public void setDbValue(DbSetGet db3, Apps val) throws KnownError {
		if (val == null) {
		...
		new IntegerCoder().setDbValue(db3,val.cls);
		new JsonCoder().setDbValue(db3,val.state);

	}

	@Override
	public String toString(Apps val) throws KnownError {
		...
		sb.append(',');sb.append(new IntegerCoder().quoted(val.cls));
		sb.append(',');sb.append(new JsonCoder().quoted(val.state));
	...
	@Override
	protected Apps inGetDbValue(DbSetGet db3) throws KnownError, JSONException {
		Apps val=new Apps();
		...
		val.cls=new IntegerCoder().getDbValue(db3);
		val.state=new JsonCoder().getDbValue(db3);

		return val;
	}

	public Apps fromString(String str) throws JSONException{
		if(str==null)
			return null;
		...
		val.cls=new IntegerCoder().fromString(ms[i++]);
		val.state=new JsonCoder().fromString(ms[i++]);
