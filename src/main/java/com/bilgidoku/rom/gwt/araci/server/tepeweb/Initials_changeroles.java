...
public class Initials_changeroles extends DbDaoBase{

	private final String callProto="select * from tepeweb.initials_changeroles(?,?,?)";
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"tepeweb.initials.changeroles", auditParams, request.getAuditParams(auditParams));
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("roles", null, null, false);
				rolessqlType.setDbValue(db3, rolessqlType.decode(val));
