...
public class Files_imageborder extends DbDaoBase{

	private final String callProto="select * from site.files_imageborder(?,?,?,?,?)";

	private final BeforeHook beforeHook=getBeforeHook("com.bilgidoku.rom.web.db.image.border");


		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder widthsqlType=new IntegerCoder();
		TypeCoder heightsqlType=new IntegerCoder();
		TypeCoder colorsqlType=new IntegerCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("width", null, null, false);
				widthsqlType.setDbValue(db3, widthsqlType.decode(val));

				val=request.getParam("height", null, null, false);
				heightsqlType.setDbValue(db3, heightsqlType.decode(val));

				val=request.getParam("color", null, null, false);
				colorsqlType.setDbValue(db3, colorsqlType.decode(val));
