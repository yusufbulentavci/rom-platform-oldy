...
public class Info_setecommerce extends DbDaoBase{

	private final String callProto="select * from site.info_setecommerce(?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder ecommercesqlType=new BooleanCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("ecommerce", null, null, false);
				ecommercesqlType.setDbValue(db3, ecommercesqlType.decode(val));
