...
public class Resources_setrtag extends DbDaoBase{

	private final String callProto="select * from rom.resources_setrtag(?,?,?::text[])";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder rtagsqlType=new ArrayCoder(new StringCoder());
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("rtag", null, null, false);
				rtagsqlType.setDbValue(db3, rtagsqlType.decode(val));
