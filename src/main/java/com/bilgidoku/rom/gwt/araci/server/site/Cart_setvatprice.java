...
public class Cart_setvatprice extends DbDaoBase{

	private final String callProto="select * from site.cart_setvatprice(?,?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder vatpricesqlType=new JsonCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("vatprice", null, null, false);
				vatpricesqlType.setDbValue(db3, vatpricesqlType.decode(val));
