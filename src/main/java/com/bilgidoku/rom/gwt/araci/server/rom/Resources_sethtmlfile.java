...
public class Resources_sethtmlfile extends DbDaoBase{

	private final String callProto="select * from rom.resources_sethtmlfile(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder htmlfilesqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("htmlfile", null, null, false);
				htmlfilesqlType.setDbValue(db3, htmlfilesqlType.decode(val));
