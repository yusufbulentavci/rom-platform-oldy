...
public class Links_neww extends DbDaoBase{

	private final String callProto="select * from site.links_new(?,?,?::rom.langs,?::rom.langs,?,?,?)";
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"site.links.new", auditParams, request.getAuditParams(auditParams));
