...
public class Files_breed extends DbDaoBase{

	private final String callProto="select * from site.files_breed(?,?,?,?,?::text[],?,?)";
		...
		TypeCoder containerappsqlType=new StringCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"site.files.breed", auditParams, request.getAuditParams(auditParams));
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("containerapp", null, null, false);
				containerappsqlType.setDbValue(db3, containerappsqlType.decode(val));
