...
public class Cart_setcalcdetails extends DbDaoBase{

	private final String callProto="select * from site.cart_setcalcdetails(?,?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder calcdetailssqlType=new JsonCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("calcdetails", null, null, false);
				calcdetailssqlType.setDbValue(db3, calcdetailssqlType.decode(val));
