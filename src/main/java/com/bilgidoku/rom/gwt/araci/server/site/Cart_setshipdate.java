...
public class Cart_setshipdate extends DbDaoBase{

	private final String callProto="select * from site.cart_setshipdate(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder shipdatesqlType=new LongCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("shipdate", null, null, false);
				shipdatesqlType.setDbValue(db3, shipdatesqlType.decode(val));
