...
public class Mails_seen extends DbDaoBase{

	private final String callProto="select * from rom.mails_seen(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder seensqlType=new BooleanCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("seen", null, null, false);
				seensqlType.setDbValue(db3, seensqlType.decode(val));
