...
public class Resources_setnestingvalue extends DbDaoBase{

	private final String callProto="select * from rom.resources_setnestingvalue(?,?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder keysqlType=new StringCoder();
		TypeCoder valuesqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("key", null, null, false);
				keysqlType.setDbValue(db3, keysqlType.decode(val));

				val=request.getParam("value", null, null, false);
				valuesqlType.setDbValue(db3, valuesqlType.decode(val));
