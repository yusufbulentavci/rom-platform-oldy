...
public class TransCoder extends
		TypeCoder<Trans> {

	public TransCoder(){
		super(true,new String[]{"ri","host_id","uri","container","html_file","modified_date","creation_date","delegated","ownercid","gid","relatedcids","mask","nesting","dbfs","weight","lexemes","rtags","aa","langcodes","title","constants"});
	}

	@Override
	public  Trans decode(Object js) throws JSONException  {
		...
		c.title=new ArrayCoder<Json>(new JsonCoder()).decode(json.get("title"));
		c.constants=new JsonCoder().decode(json.get("constants"));
	...
	@Override
	public Object encode(Trans obj) throws JSONException {
		...
		js.put("title",new ArrayCoder<Json>(new JsonCoder()).encode(obj.title));
		js.put("constants",new JsonCoder().encode(obj.constants));

		return js;
	}

	@Override
	public  Trans[] createArray(int size) {
		return new  Trans[size];
	}

	@Override
	public void setDbValue(DbSetGet db3, Trans val) throws KnownError {
		if (val == null) {
		...
		new ArrayCoder<Json>(new JsonCoder()).setDbValue(db3,val.title);
		new JsonCoder().setDbValue(db3,val.constants);

	}

	@Override
	public String toString(Trans val) throws KnownError {
		...
		sb.append(',');sb.append(new ArrayCoder<Json>(new JsonCoder()).quoted(val.title));
		sb.append(',');sb.append(new JsonCoder().quoted(val.constants));
	...
	@Override
	protected Trans inGetDbValue(DbSetGet db3) throws KnownError, JSONException {
		Trans val=new Trans();
		...
		val.title=new ArrayCoder<Json>(new JsonCoder()).getDbValue(db3);
		val.constants=new JsonCoder().getDbValue(db3);

		return val;
	}

	public Trans fromString(String str) throws JSONException{
		if(str==null)
			return null;
		...
		val.title=new ArrayCoder<Json>(new JsonCoder()).fromString(ms[i++]);
		val.constants=new JsonCoder().fromString(ms[i++]);
