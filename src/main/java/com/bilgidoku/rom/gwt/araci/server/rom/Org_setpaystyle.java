...
public class Org_setpaystyle extends DbDaoBase{

	private final String callProto="select * from rom.org_setpaystyle(?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder paystylesqlType=new JsonCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"rom.org.setpaystyle", auditParams, request.getAuditParams(auditParams));
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("paystyle", null, null, false);
				paystylesqlType.setDbValue(db3, paystylesqlType.decode(val));
