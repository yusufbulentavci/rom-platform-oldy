...
public class Stocks_setoptions extends DbDaoBase{

	private final String callProto="select * from rom.stocks_setoptions(?,?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder optionssqlType=new JsonCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("options", null, null, false);
				optionssqlType.setDbValue(db3, optionssqlType.decode(val));
