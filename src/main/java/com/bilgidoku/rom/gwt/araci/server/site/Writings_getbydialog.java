...
public class Writings_getbydialog extends DbDaoBase{

	private final String callProto="select * from site.writings_getbydialog(?,?,?::rom.langs,?::rom.langs,?)";
		...
		TypeCoder dialogidsqlType=new StringCoder();


	TypeCoder coder=new WritingsCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("dialogid", null, null, false);
				dialogidsqlType.setDbValue(db3, dialogidsqlType.decode(val));
