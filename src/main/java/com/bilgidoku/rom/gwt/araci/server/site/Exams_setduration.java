...
public class Exams_setduration extends DbDaoBase{

	private final String callProto="select * from site.exams_setduration(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder durationsqlType=new IntegerCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("duration", null, null, false);
				durationsqlType.setDbValue(db3, durationsqlType.decode(val));
