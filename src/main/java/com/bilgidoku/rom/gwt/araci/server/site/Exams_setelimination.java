...
public class Exams_setelimination extends DbDaoBase{

	private final String callProto="select * from site.exams_setelimination(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder elimsqlType=new IntegerCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("elim", null, null, false);
				elimsqlType.setDbValue(db3, elimsqlType.decode(val));
