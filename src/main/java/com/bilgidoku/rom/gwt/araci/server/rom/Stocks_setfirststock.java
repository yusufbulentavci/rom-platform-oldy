...
public class Stocks_setfirststock extends DbDaoBase{

	private final String callProto="select * from rom.stocks_setfirststock(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder firststocksqlType=new StringCoder();


	TypeCoder coder=new BooleanCoder();


	private static final int rls=RoleMask.contact|RoleMask.admin;
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("firststock", null, null, false);
				firststocksqlType.setDbValue(db3, firststocksqlType.decode(val));
