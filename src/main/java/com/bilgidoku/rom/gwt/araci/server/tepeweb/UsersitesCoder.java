...
public class UsersitesCoder extends
	...
	public UsersitesCoder(){
		super(true,new String[]{"ri","host_id","uri","container","html_file","modified_date","creation_date","delegated","ownercid","gid","relatedcids","mask","nesting","dbfs","weight","lexemes","rtags","aa","title","remotehost","remotesite","remoteworld","lastip","lastcountry","lastactivity"});
	}

	@Override
	public  Usersites decode(Object js) throws JSONException  {
	...
	@Override
	public Object encode(Usersites obj) throws JSONException {
	...
	@Override
	public void setDbValue(DbSetGet db3, Usersites val) throws KnownError {
		if (val == null) {
		...
		new IntegerCoder().setDbValue(db3,val.remotehost);
		new StringCoder().setDbValue(db3,val.remotesite);
		new StringCoder().setDbValue(db3,val.remoteworld);
		new StringCoder().setDbValue(db3,val.lastip);
		new StringCoder().setDbValue(db3,val.lastcountry);
		new StringCoder().setDbValue(db3,val.lastactivity);

	}

	@Override
	public String toString(Usersites val) throws KnownError {
		...
		sb.append(',');sb.append(new IntegerCoder().quoted(val.remotehost));
		sb.append(',');sb.append(new StringCoder().quoted(val.remotesite));
		sb.append(',');sb.append(new StringCoder().quoted(val.remoteworld));
		sb.append(',');sb.append(new StringCoder().quoted(val.lastip));
		sb.append(',');sb.append(new StringCoder().quoted(val.lastcountry));
		sb.append(',');sb.append(new StringCoder().quoted(val.lastactivity));
	...
	@Override
	protected Usersites inGetDbValue(DbSetGet db3) throws KnownError, JSONException {
		Usersites val=new Usersites();
		...
		val.remotehost=new IntegerCoder().getDbValue(db3);
		val.remotesite=new StringCoder().getDbValue(db3);
		val.remoteworld=new StringCoder().getDbValue(db3);
		val.lastip=new StringCoder().getDbValue(db3);
		val.lastcountry=new StringCoder().getDbValue(db3);
		val.lastactivity=new StringCoder().getDbValue(db3);

		return val;
	}

	public Usersites fromString(String str) throws JSONException{
		if(str==null)
			return null;
		...
		val.remotehost=new IntegerCoder().fromString(ms[i++]);
		val.remotesite=new StringCoder().fromString(ms[i++]);
		val.remoteworld=new StringCoder().fromString(ms[i++]);
		val.lastip=new StringCoder().fromString(ms[i++]);
		val.lastcountry=new StringCoder().fromString(ms[i++]);
		val.lastactivity=new StringCoder().fromString(ms[i++]);
