...
public class Cart_setdiscountprice extends DbDaoBase{

	private final String callProto="select * from site.cart_setdiscountprice(?,?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder discountpricesqlType=new JsonCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("discountprice", null, null, false);
				discountpricesqlType.setDbValue(db3, discountpricesqlType.decode(val));
