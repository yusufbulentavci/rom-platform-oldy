...
public class Apps_breed extends DbDaoBase{

	private final String callProto="select * from rom.apps_breed(?,?,?,?,?)";
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"rom.apps.breed", auditParams, request.getAuditParams(auditParams));
