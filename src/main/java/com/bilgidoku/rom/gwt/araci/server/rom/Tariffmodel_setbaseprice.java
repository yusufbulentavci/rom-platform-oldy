...
public class Tariffmodel_setbaseprice extends DbDaoBase{

	private final String callProto="select * from rom.tariffmodel_setbaseprice(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder basepricesqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("baseprice", null, null, false);
				basepricesqlType.setDbValue(db3, basepricesqlType.decode(val));
