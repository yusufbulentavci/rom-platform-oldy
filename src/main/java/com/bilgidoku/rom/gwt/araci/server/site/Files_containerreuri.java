...
public class Files_containerreuri extends DbDaoBase{

	private final String callProto="select * from site.files_containerreuri(?,?,?,?)";

	private final BeforeHook beforeHook=getBeforeHook("com.bilgidoku.rom.web.db.files.move");


		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder urisqlType=new StringCoder();
		TypeCoder uriprefixsqlType=new StringCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"site.files.containerreuri", auditParams, request.getAuditParams(auditParams));
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("uriprefix", null, null, false);
				uriprefixsqlType.setDbValue(db3, uriprefixsqlType.decode(val));
