...
public class Files_imagerotate extends DbDaoBase{

	private final String callProto="select * from site.files_imagerotate(?,?,?)";

	private final BeforeHook beforeHook=getBeforeHook("com.bilgidoku.rom.web.db.image.rotate");


		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder anglesqlType=new IntegerCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("angle", null, null, false);
				anglesqlType.setDbValue(db3, anglesqlType.decode(val));
