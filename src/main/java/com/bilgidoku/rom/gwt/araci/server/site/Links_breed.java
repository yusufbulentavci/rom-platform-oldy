...
public class Links_breed extends DbDaoBase{

	private final String callProto="select * from site.links_breed(?,?,?,?,?)";
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"site.links.breed", auditParams, request.getAuditParams(auditParams));
