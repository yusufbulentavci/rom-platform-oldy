...
public class Stocks_setvirtualsparent extends DbDaoBase{

	private final String callProto="select * from rom.stocks_setvirtualsparent(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder virtualsparentsqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("virtualsparent", null, null, false);
				virtualsparentsqlType.setDbValue(db3, virtualsparentsqlType.decode(val));
