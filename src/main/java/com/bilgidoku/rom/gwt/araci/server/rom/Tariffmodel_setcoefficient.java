...
public class Tariffmodel_setcoefficient extends DbDaoBase{

	private final String callProto="select * from rom.tariffmodel_setcoefficient(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder coefficientsqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("coefficient", null, null, false);
				coefficientsqlType.setDbValue(db3, coefficientsqlType.decode(val));
