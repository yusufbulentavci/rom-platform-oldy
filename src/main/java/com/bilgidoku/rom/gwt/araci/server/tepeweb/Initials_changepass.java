...
public class Initials_changepass extends DbDaoBase{

	private final String callProto="select * from tepeweb.initials_changepass(?,?,?)";
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"tepeweb.initials.changepass", auditParams, request.getAuditParams(auditParams));
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				credentialsqlType.setDbValue(db3, credentialsqlType.decode(val));
