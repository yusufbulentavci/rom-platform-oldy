...
public class Stocks_settm extends DbDaoBase{

	private final String callProto="select * from rom.stocks_settm(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder tmsqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("tm", null, null, false);
				tmsqlType.setDbValue(db3, tmsqlType.decode(val));
