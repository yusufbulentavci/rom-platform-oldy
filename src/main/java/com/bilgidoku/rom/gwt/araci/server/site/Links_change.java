...
public class Links_change extends DbDaoBase{

	private final String callProto="select * from site.links_change(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder linked_urisqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("linked_uri", null, null, false);
				linked_urisqlType.setDbValue(db3, linked_urisqlType.decode(val));
