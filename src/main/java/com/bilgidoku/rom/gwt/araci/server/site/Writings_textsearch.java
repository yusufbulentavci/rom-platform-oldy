...
public class Writings_textsearch extends DbDaoBase{

	private final String callProto="select * from site.writings_textsearch(?,?,?::rom.langs,?::rom.langs,?)";
		...
		TypeCoder searchsqlType=new StringCoder();


	TypeCoder coder=new ContentsCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("search", null, null, false);
				searchsqlType.setDbValue(db3, searchsqlType.decode(val));
