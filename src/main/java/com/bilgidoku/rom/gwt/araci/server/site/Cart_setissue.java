...
public class Cart_setissue extends DbDaoBase{

	private final String callProto="select * from site.cart_setissue(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder issuesqlType=new StringCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("issue", null, null, false);
				issuesqlType.setDbValue(db3, issuesqlType.decode(val));
