...
public class Containers_listsub extends DbDaoBase{

	private final String callProto="select * from rom.containers_listsub(?,?,?,?)";
		...
		TypeCoder containersqlType=new StringCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("container", null, null, false);
				containersqlType.setDbValue(db3, containersqlType.decode(val));
