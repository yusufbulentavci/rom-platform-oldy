...
public class Contacts_setworks extends DbDaoBase{

	private final String callProto="select * from rom.contacts_setworks(?,?,?::json[])";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder workssqlType=new ArrayCoder(new JsonCoder());
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("works", null, null, false);
				workssqlType.setDbValue(db3, workssqlType.decode(val));
