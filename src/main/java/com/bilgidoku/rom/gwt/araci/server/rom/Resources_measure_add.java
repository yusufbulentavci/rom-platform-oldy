...
public class Resources_measure_add extends DbDaoBase{

	private final String callProto="select * from rom.resources_measure_add(?,?,?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder datesqlType=new StringCoder();
		TypeCoder codesqlType=new StringCoder();
		TypeCoder valsqlType=new IntegerCoder();


	TypeCoder coder=new IntegerCoder();


	private static final int rls=RoleMask.owner|RoleMask.admin;
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("date", null, null, false);
				datesqlType.setDbValue(db3, datesqlType.decode(val));

				val=request.getParam("code", null, null, false);
				codesqlType.setDbValue(db3, codesqlType.decode(val));

				val=request.getParam("val", null, null, false);
				valsqlType.setDbValue(db3, valsqlType.decode(val));
