...
public class Mails_important extends DbDaoBase{

	private final String callProto="select * from rom.mails_important(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder importantsqlType=new BooleanCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("important", null, null, false);
				importantsqlType.setDbValue(db3, importantsqlType.decode(val));
