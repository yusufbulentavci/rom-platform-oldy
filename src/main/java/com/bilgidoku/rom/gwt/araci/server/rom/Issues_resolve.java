...
public class Issues_resolve extends DbDaoBase{

	private final String callProto="select * from rom.issues_resolve(?,?,?,?,?,?,?::text[],?,?::rom.langs,?::rom.langs,?,?,?)";
		...
		TypeCoder resolvedescsqlType=new StringCoder();
		TypeCoder resolvecodesqlType=new IntegerCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("resolvedesc", null, null, false);
				resolvedescsqlType.setDbValue(db3, resolvedescsqlType.decode(val));

				val=request.getParam("resolvecode", null, null, false);
				resolvecodesqlType.setDbValue(db3, resolvecodesqlType.decode(val));
