...
public class Cart_settotalprice extends DbDaoBase{

	private final String callProto="select * from site.cart_settotalprice(?,?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder totalpricesqlType=new JsonCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("totalprice", null, null, false);
				totalpricesqlType.setDbValue(db3, totalpricesqlType.decode(val));
