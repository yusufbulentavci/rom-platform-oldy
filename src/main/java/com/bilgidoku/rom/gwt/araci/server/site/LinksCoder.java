...
public class LinksCoder extends
		TypeCoder<Links> {

	public LinksCoder(){
		super(true,new String[]{"ri","host_id","uri","container","html_file","modified_date","creation_date","delegated","ownercid","gid","relatedcids","mask","nesting","dbfs","weight","lexemes","rtags","aa","title","summary","tip","icon","medium_icon","large_icon","multilang_icon","sound","langcodes","viewy","linked_uri"});
	}

	@Override
	public  Links decode(Object js) throws JSONException  {
		...
		c.linked_uri=new StringCoder().decode(json.get("linked_uri"));
	...
	@Override
	public Object encode(Links obj) throws JSONException {
		...
		js.put("linked_uri",new StringCoder().encode(obj.linked_uri));

		return js;
	}

	@Override
	public  Links[] createArray(int size) {
		return new  Links[size];
	}

	@Override
	public void setDbValue(DbSetGet db3, Links val) throws KnownError {
		if (val == null) {
		...
		new StringCoder().setDbValue(db3,val.linked_uri);

	}

	@Override
	public String toString(Links val) throws KnownError {
		...
		sb.append(',');sb.append(new StringCoder().quoted(val.linked_uri));
	...
	@Override
	protected Links inGetDbValue(DbSetGet db3) throws KnownError, JSONException {
		Links val=new Links();
		...
		val.linked_uri=new StringCoder().getDbValue(db3);

		return val;
	}

	public Links fromString(String str) throws JSONException{
		if(str==null)
			return null;
		...
		val.linked_uri=new StringCoder().fromString(ms[i++]);
