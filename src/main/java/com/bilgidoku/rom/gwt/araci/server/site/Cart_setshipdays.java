...
public class Cart_setshipdays extends DbDaoBase{

	private final String callProto="select * from site.cart_setshipdays(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder shipdayssqlType=new IntegerCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("shipdays", null, null, false);
				shipdayssqlType.setDbValue(db3, shipdayssqlType.decode(val));
