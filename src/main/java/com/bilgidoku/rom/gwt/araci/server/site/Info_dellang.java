...
public class Info_dellang extends DbDaoBase{

	private final String callProto="select * from site.info_dellang(?,?::rom.langs)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder dellangsqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("dellang", null, null, false);
				dellangsqlType.setDbValue(db3, dellangsqlType.decode(val));
