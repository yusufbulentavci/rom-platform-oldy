...
public class Yetkilendirme_trustedplatformlogin extends ServiceDaoBase{
	...
	String val;
		...
		TypeCoder accesstokensqlType=new StringCoder();
	...
	public Yetkilendirme_trustedplatformlogin(){
		method=getMethod(com.bilgidoku.rom.web.authorize.YetkilendirmeGorevlisi.class,"trustedplatformlogin");
	}

	@Override
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {

		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"/_auth.trustedplatformlogin", auditParams, request.getAuditParams(auditParams));
		...
		val=request.getParam("accesstoken", null, null, false);
		args[ind++]=accesstokensqlType.decode(val);
