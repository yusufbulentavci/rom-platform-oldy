...
public class Cart_setinvoiceaddr extends DbDaoBase{

	private final String callProto="select * from site.cart_setinvoiceaddr(?,?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder invoiceaddrsqlType=new JsonCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("invoiceaddr", null, null, false);
				invoiceaddrsqlType.setDbValue(db3, invoiceaddrsqlType.decode(val));
