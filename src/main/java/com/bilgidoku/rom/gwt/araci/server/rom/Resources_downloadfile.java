...
public class Resources_downloadfile extends DbDaoBase{

	private final String callProto="select * from rom.resources_downloadfile(?,?,?,?)";

	private final BeforeHook beforeHook=getBeforeHook("com.bilgidoku.rom.web.db.files.dbfsdownload");


		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder download_urisqlType=new StringCoder();
		TypeCoder fnsqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("fn", null, null, false);
				fnsqlType.setDbValue(db3, fnsqlType.decode(val));
