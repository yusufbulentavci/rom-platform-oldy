...
public class Stocks_setalternatives extends DbDaoBase{

	private final String callProto="select * from rom.stocks_setalternatives(?,?,?::text[])";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder alternativessqlType=new ArrayCoder(new StringCoder());
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("alternatives", null, null, false);
				alternativessqlType.setDbValue(db3, alternativessqlType.decode(val));
