...
public class Trans_changeconstants extends DbDaoBase{

	private final String callProto="select * from rom.trans_changeconstants(?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder constantssqlType=new JsonCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("constants", null, null, false);
				constantssqlType.setDbValue(db3, constantssqlType.decode(val));
