...
public class Tags_neww extends DbDaoBase{

	private final String callProto="select * from rom.tags_new(?,?,?::rom.langs,?::rom.langs,?,?,?::text[])";
		...
		TypeCoder parenttagssqlType=new ArrayCoder(new StringCoder());
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"rom.tags.new", auditParams, request.getAuditParams(auditParams));
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("parenttags", null, null, false);
				parenttagssqlType.setDbValue(db3, parenttagssqlType.decode(val));
