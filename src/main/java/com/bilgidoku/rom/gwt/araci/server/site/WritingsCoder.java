...
public class WritingsCoder extends
	...
	public WritingsCoder(){
		super(true,new String[]{"ri","host_id","uri","container","html_file","modified_date","creation_date","delegated","ownercid","gid","relatedcids","mask","nesting","dbfs","weight","lexemes","rtags","aa","title","summary","tip","icon","medium_icon","large_icon","multilang_icon","sound","langcodes","viewy","spot","body","menu1","dialog_uri","carray","tags","stock_uri"});
	}

	@Override
	public  Writings decode(Object js) throws JSONException  {
	...
	@Override
	public Object encode(Writings obj) throws JSONException {
	...
	@Override
	public void setDbValue(DbSetGet db3, Writings val) throws KnownError {
		if (val == null) {
		...
		new ArrayCoder<Json>(new JsonCoder()).setDbValue(db3,val.spot);
		new ArrayCoder<Json>(new JsonCoder()).setDbValue(db3,val.body);
		new StringCoder().setDbValue(db3,val.menu1);
		new StringCoder().setDbValue(db3,val.dialog_uri);
		new ArrayCoder<String>(new StringCoder()).setDbValue(db3,val.carray);
		new ArrayCoder<String>(new StringCoder()).setDbValue(db3,val.tags);
		new StringCoder().setDbValue(db3,val.stock_uri);

	}

	@Override
	public String toString(Writings val) throws KnownError {
		...
		sb.append(',');sb.append(new ArrayCoder<Json>(new JsonCoder()).quoted(val.spot));
		sb.append(',');sb.append(new ArrayCoder<Json>(new JsonCoder()).quoted(val.body));
		sb.append(',');sb.append(new StringCoder().quoted(val.menu1));
		sb.append(',');sb.append(new StringCoder().quoted(val.dialog_uri));
		sb.append(',');sb.append(new ArrayCoder<String>(new StringCoder()).quoted(val.carray));
		sb.append(',');sb.append(new ArrayCoder<String>(new StringCoder()).quoted(val.tags));
		sb.append(',');sb.append(new StringCoder().quoted(val.stock_uri));
	...
	@Override
	protected Writings inGetDbValue(DbSetGet db3) throws KnownError, JSONException {
		Writings val=new Writings();
		...
		val.spot=new ArrayCoder<Json>(new JsonCoder()).getDbValue(db3);
		val.body=new ArrayCoder<Json>(new JsonCoder()).getDbValue(db3);
		val.menu1=new StringCoder().getDbValue(db3);
		val.dialog_uri=new StringCoder().getDbValue(db3);
		val.carray=new ArrayCoder<String>(new StringCoder()).getDbValue(db3);
		val.tags=new ArrayCoder<String>(new StringCoder()).getDbValue(db3);
		val.stock_uri=new StringCoder().getDbValue(db3);

		return val;
	}

	public Writings fromString(String str) throws JSONException{
		if(str==null)
			return null;
		...
		val.spot=new ArrayCoder<Json>(new JsonCoder()).fromString(ms[i++]);
		val.body=new ArrayCoder<Json>(new JsonCoder()).fromString(ms[i++]);
		val.menu1=new StringCoder().fromString(ms[i++]);
		val.dialog_uri=new StringCoder().fromString(ms[i++]);
		val.carray=new ArrayCoder<String>(new StringCoder()).fromString(ms[i++]);
		val.tags=new ArrayCoder<String>(new StringCoder()).fromString(ms[i++]);
		val.stock_uri=new StringCoder().fromString(ms[i++]);
