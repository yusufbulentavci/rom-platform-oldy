...
public class Files_list extends DbDaoBase{

	private final String callProto="select * from site.files_list(?,?,?::rom.langs,?::rom.langs,?,?)";
	...
	TypeCoder coder=new FilesCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				offsetsqlType.setDbValue(db3, offsetsqlType.decode(val));

				val=request.getParam("limit", null, null, false);
				limitsqlType.setDbValue(db3, limitsqlType.decode(val));
