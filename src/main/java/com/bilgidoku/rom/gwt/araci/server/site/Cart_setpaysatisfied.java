...
public class Cart_setpaysatisfied extends DbDaoBase{

	private final String callProto="select * from site.cart_setpaysatisfied(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder paysatisfiedsqlType=new BooleanCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("paysatisfied", null, null, false);
				paysatisfiedsqlType.setDbValue(db3, paysatisfiedsqlType.decode(val));
