...
public class Cart_setpayments extends DbDaoBase{

	private final String callProto="select * from site.cart_setpayments(?,?,?::json[])";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder paymentssqlType=new ArrayCoder(new JsonCoder());
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("payments", null, null, false);
				paymentssqlType.setDbValue(db3, paymentssqlType.decode(val));
