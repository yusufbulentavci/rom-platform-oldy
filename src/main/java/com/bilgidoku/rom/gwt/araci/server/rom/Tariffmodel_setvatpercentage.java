...
public class Tariffmodel_setvatpercentage extends DbDaoBase{

	private final String callProto="select * from rom.tariffmodel_setvatpercentage(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder vatpercentagesqlType=new IntegerCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("vatpercentage", null, null, false);
				vatpercentagesqlType.setDbValue(db3, vatpercentagesqlType.decode(val));
