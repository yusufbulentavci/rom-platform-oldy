...
public class Cart_setitems extends DbDaoBase{

	private final String callProto="select * from site.cart_setitems(?,?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder itemssqlType=new JsonCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("items", null, null, false);
				itemssqlType.setDbValue(db3, itemssqlType.decode(val));
