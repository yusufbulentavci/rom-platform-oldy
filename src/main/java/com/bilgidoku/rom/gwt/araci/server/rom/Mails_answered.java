...
public class Mails_answered extends DbDaoBase{

	private final String callProto="select * from rom.mails_answered(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder answeredsqlType=new BooleanCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("answered", null, null, false);
				answeredsqlType.setDbValue(db3, answeredsqlType.decode(val));
