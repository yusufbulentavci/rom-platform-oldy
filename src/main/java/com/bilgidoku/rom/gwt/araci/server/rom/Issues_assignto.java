...
public class Issues_assignto extends DbDaoBase{

	private final String callProto="select * from rom.issues_assignto(?,?,?)";
	...
	TypeCoder coder=new StringCoder();
			...
			private static String[] auditParams=new String[]{"contact"};
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"rom.issues.assignto", auditParams, request.getAuditParams(auditParams));
