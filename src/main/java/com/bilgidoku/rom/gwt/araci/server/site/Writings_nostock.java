...
public class Writings_nostock extends DbDaoBase{

	private final String callProto="select * from site.writings_nostock(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder delsqlType=new BooleanCoder();


	TypeCoder coder=new StringCoder();

			private static String[] auditParams=new String[]{"self"};


	@Override
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {

		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"site.writings.nostock", auditParams, request.getAuditParams(auditParams));
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("del", null, null, false);
				delsqlType.setDbValue(db3, delsqlType.decode(val));
