...
public class Contacts_adminchangepwd extends DbDaoBase{

	private final String callProto="select * from rom.contacts_adminchangepwd(?,?,?)";
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				cidsqlType.setDbValue(db3, cidsqlType.decode(val));
