...
public class Info_textfont extends DbDaoBase{

	private final String callProto="select * from site.info_textfont(?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder textfontsqlType=new JsonCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("textfont", null, null, false);
				textfontsqlType.setDbValue(db3, textfontsqlType.decode(val));
