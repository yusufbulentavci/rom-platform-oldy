...
public class Containers_reuri extends DbDaoBase{

	private final String callProto="select * from rom.containers_reuri(?,?,?,?)";
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"rom.containers.reuri", auditParams, request.getAuditParams(auditParams));
