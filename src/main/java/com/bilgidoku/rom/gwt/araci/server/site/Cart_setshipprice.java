...
public class Cart_setshipprice extends DbDaoBase{

	private final String callProto="select * from site.cart_setshipprice(?,?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder shippricesqlType=new JsonCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("shipprice", null, null, false);
				shippricesqlType.setDbValue(db3, shippricesqlType.decode(val));
