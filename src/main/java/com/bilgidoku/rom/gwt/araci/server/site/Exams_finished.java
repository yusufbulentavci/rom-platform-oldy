...
public class Exams_finished extends DbDaoBase{

	private final String callProto="select * from site.exams_finished(?,?,?,?::int4[])";
		...
		TypeCoder responsessqlType=new ArrayCoder(new IntegerCoder());
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("responses", null, null, false);
				responsessqlType.setDbValue(db3, responsessqlType.decode(val));
