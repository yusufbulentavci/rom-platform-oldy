...
public class Cart_setpayconfirmed extends DbDaoBase{

	private final String callProto="select * from site.cart_setpayconfirmed(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder payconfirmedsqlType=new BooleanCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("payconfirmed", null, null, false);
				payconfirmedsqlType.setDbValue(db3, payconfirmedsqlType.decode(val));
