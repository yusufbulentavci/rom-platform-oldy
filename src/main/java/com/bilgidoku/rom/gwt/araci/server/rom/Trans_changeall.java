...
public class Trans_changeall extends DbDaoBase{

	private final String callProto="select * from rom.trans_changeall(?,?,?::rom.langs[],?::json[])";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder langssqlType=new ArrayCoder(new StringCoder());
		TypeCoder titlesqlType=new ArrayCoder(new JsonCoder());
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("langs", null, null, false);
				langssqlType.setDbValue(db3, langssqlType.decode(val));
