...
public class Resources_setweight extends DbDaoBase{

	private final String callProto="select * from rom.resources_setweight(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder weightsqlType=new IntegerCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("weight", null, null, false);
				weightsqlType.setDbValue(db3, weightsqlType.decode(val));
