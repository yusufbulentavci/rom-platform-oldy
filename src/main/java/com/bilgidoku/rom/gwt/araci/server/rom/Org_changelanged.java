...
public class Org_changelanged extends DbDaoBase{

	private final String callProto="select * from rom.org_changelanged(?,?,?::rom.langs,?::rom.langs,?,?)";
		...
		TypeCoder cpicsqlType=new StringCoder();
		TypeCoder crssqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("cpic", null, null, false);
				cpicsqlType.setDbValue(db3, cpicsqlType.decode(val));

				val=request.getParam("crs", null, null, false);
				crssqlType.setDbValue(db3, crssqlType.decode(val));
