...
public class Styles_neww extends DbDaoBase{

	private final String callProto="select * from rom.styles_new(?,?,?,?)";
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"rom.styles.new", auditParams, request.getAuditParams(auditParams));
