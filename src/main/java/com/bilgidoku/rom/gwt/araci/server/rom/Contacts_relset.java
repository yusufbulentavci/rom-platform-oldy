...
public class Contacts_relset extends DbDaoBase{

	private final String callProto="select * from rom.contacts_relset(?,?,?,?)";
		...
		TypeCoder tonesqlType=new IntegerCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("tone", null, null, false);
				tonesqlType.setDbValue(db3, tonesqlType.decode(val));
