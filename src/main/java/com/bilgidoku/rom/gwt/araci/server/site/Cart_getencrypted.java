...
public class Cart_getencrypted extends DbDaoBase{

	private final String callProto="select * from site.cart_getencrypted(?,?,?,?,?)";


	private final AfterHook afterHook=getAfterHook("com.bilgidoku.rom.web.db.cart.encrypt");
		...
		TypeCoder forwhatsqlType=new StringCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("forwhat", null, null, false);
				forwhatsqlType.setDbValue(db3, forwhatsqlType.decode(val));
