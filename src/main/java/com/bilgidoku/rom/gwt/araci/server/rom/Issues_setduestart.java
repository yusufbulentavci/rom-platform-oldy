...
public class Issues_setduestart extends DbDaoBase{

	private final String callProto="select * from rom.issues_setduestart(?,?,?::timestamp)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder tssqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("ts", null, null, false);
				tssqlType.setDbValue(db3, tssqlType.decode(val));
