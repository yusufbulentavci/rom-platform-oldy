...
public class Waiting_setvalidperiod extends DbDaoBase{

	private final String callProto="select * from rom.waiting_setvalidperiod(?,?,?::timestamp,?::timestamp)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder valid_aftersqlType=new StringCoder();
		TypeCoder valid_beforesqlType=new StringCoder();


	TypeCoder coder=new StringCoder();


	private static final int rls=RoleMask.owner;
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("valid_after", null, null, false);
				valid_aftersqlType.setDbValue(db3, valid_aftersqlType.decode(val));

				val=request.getParam("valid_before", null, null, false);
				valid_beforesqlType.setDbValue(db3, valid_beforesqlType.decode(val));
