...
public class Issues_changedesc extends DbDaoBase{

	private final String callProto="select * from rom.issues_changedesc(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder descriptionsqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("description", null, null, false);
				descriptionsqlType.setDbValue(db3, descriptionsqlType.decode(val));
