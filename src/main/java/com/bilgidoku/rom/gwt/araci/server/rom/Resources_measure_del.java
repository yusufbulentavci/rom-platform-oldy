...
public class Resources_measure_del extends DbDaoBase{

	private final String callProto="select * from rom.resources_measure_del(?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder idsqlType=new StringCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("id", null, null, false);
				idsqlType.setDbValue(db3, idsqlType.decode(val));
