...
public class Info_logo extends DbDaoBase{

	private final String callProto="select * from site.info_logo(?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder logoimgsqlType=new JsonCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("logoimg", null, null, false);
				logoimgsqlType.setDbValue(db3, logoimgsqlType.decode(val));
