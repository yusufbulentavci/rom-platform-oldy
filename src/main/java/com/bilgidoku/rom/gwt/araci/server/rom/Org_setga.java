...
public class Org_setga extends DbDaoBase{

	private final String callProto="select * from rom.org_setga(?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder gasqlType=new StringCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("ga", null, null, false);
				gasqlType.setDbValue(db3, gasqlType.decode(val));
