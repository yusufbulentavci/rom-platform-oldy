...
public class Dialogs_cmd extends DbDaoBase{

	private final String callProto="select * from rom.dialogs_cmd(?,?,?,?,?,?::json)";
		...
		TypeCoder cmdsqlType=new StringCoder();
		TypeCoder strsqlType=new StringCoder();
		TypeCoder moresqlType=new JsonCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("str", null, null, false);
				strsqlType.setDbValue(db3, strsqlType.decode(val));

				val=request.getParam("more", null, null, false);
				moresqlType.setDbValue(db3, moresqlType.decode(val));
