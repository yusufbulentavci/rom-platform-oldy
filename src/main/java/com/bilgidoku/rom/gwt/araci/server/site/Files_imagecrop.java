...
public class Files_imagecrop extends DbDaoBase{

	private final String callProto="select * from site.files_imagecrop(?,?,?,?,?,?)";

	private final BeforeHook beforeHook=getBeforeHook("com.bilgidoku.rom.web.db.image.crop");
		...
		TypeCoder rightsqlType=new IntegerCoder();
		TypeCoder bottomsqlType=new IntegerCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("right", null, null, false);
				rightsqlType.setDbValue(db3, rightsqlType.decode(val));

				val=request.getParam("bottom", null, null, false);
				bottomsqlType.setDbValue(db3, bottomsqlType.decode(val));
