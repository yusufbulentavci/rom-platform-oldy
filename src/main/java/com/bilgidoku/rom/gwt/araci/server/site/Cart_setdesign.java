...
public class Cart_setdesign extends DbDaoBase{

	private final String callProto="select * from site.cart_setdesign(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder designsqlType=new BooleanCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("design", null, null, false);
				designsqlType.setDbValue(db3, designsqlType.decode(val));
