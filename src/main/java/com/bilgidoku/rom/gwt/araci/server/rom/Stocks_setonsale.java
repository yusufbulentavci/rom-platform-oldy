...
public class Stocks_setonsale extends DbDaoBase{

	private final String callProto="select * from rom.stocks_setonsale(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder onsalesqlType=new BooleanCoder();
	...
	request.checkRole(rls);
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("onsale", null, null, false);
				onsalesqlType.setDbValue(db3, onsalesqlType.decode(val));
