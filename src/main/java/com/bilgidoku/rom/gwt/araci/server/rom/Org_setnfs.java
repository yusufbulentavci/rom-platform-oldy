...
public class Org_setnfs extends DbDaoBase{

	private final String callProto="select * from rom.org_setnfs(?,?::json)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder nfssqlType=new JsonCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("nfs", null, null, false);
				nfssqlType.setDbValue(db3, nfssqlType.decode(val));
