...
public class Org_setstartssl extends DbDaoBase{

	private final String callProto="select * from rom.org_setstartssl(?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder startsslsqlType=new StringCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"rom.org.setstartssl", auditParams, request.getAuditParams(auditParams));
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("startssl", null, null, false);
				startsslsqlType.setDbValue(db3, startsslsqlType.decode(val));
