...
public class ListsCoder extends
		TypeCoder<Lists> {

	public ListsCoder(){
		super(true,new String[]{"ri","host_id","uri","container","html_file","modified_date","creation_date","delegated","ownercid","gid","relatedcids","mask","nesting","dbfs","weight","lexemes","rtags","aa","title","summary","tip","icon","medium_icon","large_icon","multilang_icon","sound","langcodes","viewy","content_ids"});
	}

	@Override
	public  Lists decode(Object js) throws JSONException  {
		...
		c.content_ids=new ArrayCoder<String>(new StringCoder()).decode(json.get("content_ids"));
	...
	@Override
	public Object encode(Lists obj) throws JSONException {
		...
		js.put("content_ids",new ArrayCoder<String>(new StringCoder()).encode(obj.content_ids));

		return js;
	}

	@Override
	public  Lists[] createArray(int size) {
		return new  Lists[size];
	}

	@Override
	public void setDbValue(DbSetGet db3, Lists val) throws KnownError {
		if (val == null) {
		...
		new ArrayCoder<String>(new StringCoder()).setDbValue(db3,val.content_ids);

	}

	@Override
	public String toString(Lists val) throws KnownError {
		...
		sb.append(',');sb.append(new ArrayCoder<String>(new StringCoder()).quoted(val.content_ids));
	...
	@Override
	protected Lists inGetDbValue(DbSetGet db3) throws KnownError, JSONException {
		Lists val=new Lists();
		...
		val.content_ids=new ArrayCoder<String>(new StringCoder()).getDbValue(db3);

		return val;
	}

	public Lists fromString(String str) throws JSONException{
		if(str==null)
			return null;
		...
		val.content_ids=new ArrayCoder<String>(new StringCoder()).fromString(ms[i++]);
