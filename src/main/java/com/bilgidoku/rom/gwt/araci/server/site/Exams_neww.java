...
public class Exams_neww extends DbDaoBase{

	private final String callProto="select * from site.exams_new(?,?,?::rom.langs,?::rom.langs,?,?)";
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"site.exams.new", auditParams, request.getAuditParams(auditParams));
