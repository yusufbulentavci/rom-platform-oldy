...
public class Cart_setshipref extends DbDaoBase{

	private final String callProto="select * from site.cart_setshipref(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder shiprefsqlType=new StringCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("shipref", null, null, false);
				shiprefsqlType.setDbValue(db3, shiprefsqlType.decode(val));
