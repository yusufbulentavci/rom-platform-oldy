...
public class Cart_setinvoicesent extends DbDaoBase{

	private final String callProto="select * from site.cart_setinvoicesent(?,?,?)";



		TypeCoder hostsqlType=new IntegerCoder();
		TypeCoder selfsqlType=new StringCoder();
		TypeCoder invoicesentsqlType=new BooleanCoder();
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		try (DbThree db3 = new DbThree(callProto)) {
			String val;
				...
				val=request.getParam("invoicesent", null, null, false);
				invoicesentsqlType.setDbValue(db3, invoicesentsqlType.decode(val));
