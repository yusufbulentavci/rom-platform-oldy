...
public class Writings_containerreuri extends DbDaoBase{

	private final String callProto="select * from site.writings_containerreuri(?,?,?,?)";
	...
	public Object call(CallInteraction request) throws KnownError, NotInlineMethodException, SecurityError, ParameterError {
		...
		DenetimGorevlisi.tek().write(request.getHostId(),request.getSession().getCid(),request.getUri(),"site.writings.containerreuri", auditParams, request.getAuditParams(auditParams));
