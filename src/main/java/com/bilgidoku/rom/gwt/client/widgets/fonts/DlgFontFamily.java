...
public class DlgFontFamily extends ActionBarDlg {
	...
	public DlgFontFamily(String text2) {		
		super("Fontlar", null, null);
		text = text2;
		ta.setValue(text2);
		run();
	}
	...
	private List<String> createList() {
		...
		allFonts.add("BreeSerif-Regular");
...
//		allFonts.add("WarsawGothicOuLn");arr
	...
	private class FontCell extends AbstractCell<String> {
		...
		public void render(Context context, String row, SafeHtmlBuilder sb) {
			...
			sb.appendHtmlConstant("<div style='width: 580px;position:relative;overflow:hidden;'>"
					...
					+ ";\">" + text + "<span style='font-size:13px;'>  " + row + "</span></div></div>");

		}
	}

	public void selectFont(String activeFont) {
		int i = ClientUtil.findInList(meys, activeFont);
		offset = i - i % 10;
		updateView();		
	}
