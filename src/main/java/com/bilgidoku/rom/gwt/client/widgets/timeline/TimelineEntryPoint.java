...
public class TimelineEntryPoint implements EntryPoint {
	...
	public void onModuleLoad() {
		...
		Runnable onLoadCallback = new Runnable() {
			public void run() {
				RomEntryPoint.one.addToRootPanel("txtStartDate",txtStartDate);
				RomEntryPoint.one.addToRootPanel("txtEndDate", txtEndDate);
				RomEntryPoint.one.addToRootPanel("btnSetRange", btnSetRange);
				RomEntryPoint.one.addToRootPanel("lblCustomTime", lblCustomTime);
				RomEntryPoint.one.addToRootPanel("chkConfirmChange", chkConfirmChange);

				RomEntryPoint.one.addToRootPanel("btnRedraw", btnRedraw);
				...
				RomEntryPoint.one.addToRootPanel("btnAddData", btnAddData);
				...
				RomEntryPoint.one.addToRootPanel("mytimeline", timeline);
			...
			public void onSelect(SelectEvent event) {
				...
				if (sel.length() > 0) {
					...
					String info = "Select event " + String.valueOf(row)
							+ " selected";
					RomEntryPoint.one.addToRootPanel("lblInfo",
							new Label(info));
				} else {
