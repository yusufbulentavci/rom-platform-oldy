...
package com.bilgidoku.rom.epostatemel.mail.imap.protocol;

import java.util.List;
import java.util.ArrayList;

import com.bilgidoku.rom.epostatemel.javam.mail.internet.ParameterList;
import com.bilgidoku.rom.epostatemel.mail.iap.*;
...
public class BODYSTRUCTURE implements Item {
	...
	if (parseDebug)
	    Sistem.outln("DEBUG IMAP: parsing BODYSTRUCTURE");
	msgno = r.getNumber();
	if (parseDebug)
	    Sistem.outln("DEBUG IMAP: msgno " + msgno);
	...
	if (r.peekByte() == '(') { // multipart
	    if (parseDebug)
		Sistem.outln("DEBUG IMAP: parsing multipart");
	    type = "multipart";
	    processedType = MULTI;
	    ...
	    if (parseDebug)
		Sistem.outln("DEBUG IMAP: subtype " + subtype);

	    if (r.readByte() == ')') { // done
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: parse DONE");
		return;
	    }
	    ...
	    if (parseDebug)
		Sistem.outln("DEBUG IMAP: parsing extension data");
	    // Body parameters
	    cParams = parseParameters(r);
	    if (r.readByte() == ')') { // done
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: body parameters DONE");
		return;
	    }
	    ...
	    if (b == '(') {
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: parse disposition");
		disposition = r.readString();
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: disposition " +
							disposition);
		...
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: disposition DONE");
	    } else if (b == 'N' || b == 'n') {
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: disposition NIL");
		r.skip(2); // skip 'NIL'
	    } else {
		throw new ParsingException(
		    "BODYSTRUCTURE parse error: " +
	    ...
	    if ((b = r.readByte()) == ')') {
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: no body-fld-lang");
		return; // done
	    }
		...
		if (l != null) {
		    ...
		    language = la;
		    if (parseDebug)
			Sistem.outln("DEBUG IMAP: language " + l);
		}
	    }
	    ...
	    if (parseDebug)
		Sistem.outln("DEBUG IMAP: single part");
	    type = r.readString();
	    if (parseDebug)
		Sistem.outln("DEBUG IMAP: type " + type);
	    ...
	    cParams = parseParameters(r);
	    if (parseDebug)
		Sistem.outln("DEBUG IMAP: cParams " + cParams);
	    id = r.readString();
	    if (parseDebug)
		Sistem.outln("DEBUG IMAP: id " + id);
	    description = r.readString();
	    if (parseDebug)
		Sistem.outln("DEBUG IMAP: description " + description);
	    /*
		...
		encoding = null;
	    if (parseDebug)
		Sistem.outln("DEBUG IMAP: encoding " + encoding);
	    size = r.readNumber();
	    if (parseDebug)
		Sistem.outln("DEBUG IMAP: size " + size);
	    if (size < 0)
		throw new ParsingException(
		...
		lines = r.readNumber();
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: lines " + lines);
		if (lines < 0)
		    throw new ParsingException(
	    ...
	    if (r.readByte() == ')') {
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: no MD5 DONE");
		return; // done
	    }
		...
		dParams = parseParameters(r);
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: dParams " + dParams);
		if (r.readByte() != ')') // eat the end ')'
		    throw new ParsingException(
			"BODYSTRUCTURE parse error: " +
		...
		language = r.readStringList();
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: language len " +
							language.length);
		...
		parseBodyExtension(r);
	    if (parseDebug)
		Sistem.outln("DEBUG IMAP: all DONE");
	}
    }
	...
	if (b == '(') {
		...
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: parameter name " + name);
		if (name == null)
		    throw new ParsingException(
			"BODYSTRUCTURE parse error: " +
			type + "/" + subtype + ": " +
		...
		if (parseDebug)
		    Sistem.outln("DEBUG IMAP: parameter value " + value);
		list.set(name, value);
	    } while (r.readByte() != ')');
	    list.combineSegments();
	} else if (b == 'N' || b == 'n') {
	    if (parseDebug)
		Sistem.outln("DEBUG IMAP: parameter list NIL");
	    r.skip(2);
	} else
