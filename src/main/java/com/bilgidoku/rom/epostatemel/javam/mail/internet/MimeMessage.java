...
import com.bilgidoku.rom.epostatemel.javam.mail.Folder;
import com.bilgidoku.rom.epostatemel.javam.mail.FolderClosedException;
import com.bilgidoku.rom.epostatemel.javam.mail.IllegalWriteException;
import com.bilgidoku.rom.epostatemel.javam.mail.Message;
import com.bilgidoku.rom.epostatemel.javam.mail.MessageRemovedException;
import com.bilgidoku.rom.epostatemel.javam.mail.MessagingException;
import com.bilgidoku.rom.epostatemel.javam.mail.Multipart;
import com.bilgidoku.rom.epostatemel.javam.mail.Session;
import com.bilgidoku.rom.epostatemel.javam.mail.util.SharedByteArrayInputStream;
import com.bilgidoku.rom.epostatemel.mail.util.ASCIIUtility;
import com.bilgidoku.rom.epostatemel.mail.util.FolderClosedIOException;
import com.bilgidoku.rom.epostatemel.mail.util.LineOutputStream;
import com.bilgidoku.rom.epostatemel.mail.util.MessageRemovedIOException;
import com.bilgidoku.rom.epostatemel.mail.util.MimeUtil;
import com.bilgidoku.rom.epostatemel.mail.util.PropUtil;
 ...
 * @see com.bilgidoku.rom.epostatemel.javam.mail.internet.MimeUtility
 * @see com.bilgidoku.rom.epostatemel.javam.mail.Part
 * @see com.bilgidoku.rom.epostatemel.javam.mail.Message
 * @see com.bilgidoku.rom.epostatemel.javam.mail.internet.MimePart
 * @see com.bilgidoku.rom.epostatemel.javam.mail.internet.InternetAddress
 */
...
public class MimeMessage extends Message implements MimePart {
	 ...
	 * another such stream representing the content of this message. In this
	...
	/**
	 * A flag indicating whether the message has been modified. If the message
	 * has not been modified, any data in the <code>content</code> array is
	 * assumed to be valid and is used directly in the <code>writeTo</code>
	 * method. This flag is set to true when an empty message is created or when
	...
	/**
	 * Does the <code>saveChanges</code> method need to be called on this
	 * message? This flag is set to false by the public constructor and set to
	 * true by the <code>saveChanges</code> method. The <code>writeTo</code>
	 * method checks this flag and calls the <code>saveChanges</code> method as
	 * necessary. This avoids the common mistake of forgetting to call the
	...
	/**
	 * If our content is a Multipart or Message object, we save it the first
	...
	public MimeMessage(String subject, String body, String sender, List<MailAddress> rcpts) throws AddressException,
			MessagingException {
		this(Session.getInstance());
		setFrom(new InternetAddress(sender));
		for (MailAddress mailAddress : rcpts) {
			addRecipient(Message.RecipientType.TO, new InternetAddress(mailAddress.toString()));
		}
		setSubject(subject);
		setText(body, "UTF8");
	}

	// public MimeMessage(AttachDeal attach, JSONObject jo) throws KnownError {
	// super(Session.getInstance());
	// modified = true;
	// try {
	// flags=new Flags();
	// flags.setSystemFlags(jo.getInt("flags"));
	// headers = new InternetHeaders();
	// JSONObject hd=jo.getJSONObject("headers");
	// Iterator it=hd.keys();
	// while(it.hasNext()){
	// String key=(String) it.next();
	// String value=hd.getString(key);
	// headers.addHeader(key, value);
	// }
	// String dbfs=jo.optString("dbfs");
	// if(dbfs!=null){
	// File file=attach.get(dbfs);
	// DataSource ds=new FileDataSource(file);
	// setDataHandler(new DataHandler(ds));
	// setFileName(HostingFS.noDbfsName(dbfs));
	// return;
	// }
	// JSONArray embed=jo.optJSONArray("embed");
	// if(embed!=null){
	// MimeMultipart mmp=new MimeMultipart(attach, embed);
	// setContent(mmp);
	// return;
	// }
	// String cont=jo.optString("content");
	// if(cont!=null){
	...
	//
	// @Override
	// public JSONObject getJson(AttachDeal attachAdder) throws KnownError {
	// try {
	// JSONObject ret = new JSONObject();
	// ret.put("flags", flags.getSystemFlagsInt());
	// JSONObject hs = new JSONObject();
	// ret.put("headers", hs);
	// Enumeration<Header> all = headers.getAllHeaders();
	// while (all.hasMoreElements()) {
	// Header h = all.nextElement();
	// hs.put(h.getName(), h.getValue());
	// }
	//
	...
	// if (fileName == null) {
	// com.bilgidoku.rom.gunluk.Sistem.errln("Content-disposition attachment without file name, ignoring:"
	// + hs.getString("Content-Disposition"));
	// } else {
	// InputStream is = getContentStream();
	// String dbfsname = attachAdder.make(is, fileName);
	// ret.put("dbfs", dbfsname);
	// return ret;
	// }
	// }
	// }
	...
	// JSONArray mp = ((MimeMultipart) o).getJson(attachAdder);
	// ret.put("embed", mp);
	// } else {
	// byte[] c=content;
	// if(c==null){
	...
	//
	//
	// public static List<MailAddress> headerMailAddr(JSONObject jo, String
	// field) throws JSONException{
	// JSONObject header = jo.getJSONObject("headers");
	// String k=header.optString(field);
	// if(k==null)
	// return null;
	// k=k.trim();
	// if(k.length()==0)
	// return null;
	// String[] kk = k.split(",");
	// List<MailAddress> ret=new ArrayList<MailAddress>();
	// for(int i=0; i<kk.length; i++){
	// try {
	// MailAddress ma = new MailAddress(kk[i]);
	// ret.add(ma);
	 ...
	 * specified MIME InputStream. The InputStream will be left positioned at
	 * the end of the data for the message. Note that the input stream parse is
	 * done within this constructor itself.
	 * <p>
	 * 
	 ...
	 * 
	 * @param folder
	 *            the Folder this message is from
	 * @param msgnum
	 ...
	 * <p>
	 * 
	 * This method is intended for use by subclasses that need to control when
	 * the InputStream is parsed.
	 * 
	 * @param is
	 *            The message input stream
	 * @exception MessagingException
	 ...
	 * field is absent, the "Sender" header field is used. If the "Sender"
	 * header field is also absent, <code>null</code> is returned.
	 * <p>
	 * 
	...
	public InternetAddress[] getInternetFrom() throws MessagingException {
		InternetAddress[] a = getInternetAddressHeader("From");
		if (a == null)
			a = getInternetAddressHeader("Sender");
	...
	/**
	 * Set the RFC 822 "From" header field. Any existing values are replaced
	 * with the given addresses. If address is <code>null</code>, this header is
	 * removed.
	 * 
	 * @param address
	 *            the sender(s) of this message
	 * @exception IllegalWriteException
	 *                if the underlying implementation does not support
	 *                modification of existing values
	 * @exception IllegalStateException
	...
	public Address getSenderRom() throws KnownError{
		Address[] a;
		try {
			a = getAddressHeader("Sender");
			if (a == null || a.length == 0)
			...
			throw new KnownError("Get Sender header failed",e);
		}
	...
	/**
	 * Returns the recepients specified by the type. The mapping between the
	 * type and the corresponding RFC 822 header is as follows:
	 * 
	 * <pre>
	 ...
	 * @see #headers
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.Message.RecipientType#TO
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.Message.RecipientType#CC
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.Message.RecipientType#BCC
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.internet.MimeMessage.RecipientType#NEWSGROUPS
	 */
	...
	public NewsAddress[] getNewsRecipients() throws MessagingException {
		String s = getHeader("Newsgroups", ",");
		return (s == null) ? null : NewsAddress.parse(s);
	}

	public InternetAddress[] getInternetRecipients(Message.RecipientType type) throws MessagingException {
		return getInternetAddressHeader(getHeaderName(type));
	}

	private InternetAddress[] getInternetAddressHeader(String name) throws MessagingException {
		String s = getHeader(name, ",");
		return (s == null) ? null : InternetAddress.parseHeader(s, strict);
	}

	/**
	 * Get all the recipient addresses for the message. Extracts the TO, CC,
	 * BCC, and NEWSGROUPS recipients.
	 * 
	 * @return array of Address objects
	 * @exception MessagingException
	 *                for failures
	 ...
	 * is unavailable or its value is absent, then the <code>getFrom</code>
	 ...
	 * into Unicode. If the decoding or conversion fails, the raw data is
	 * returned as is.
	 * <p>
	 * 
	 * This implementation uses the <code>getHeader</code> method to obtain the
	 ...
	 * characters, it will be encoded using the platform's default charset. If
	 * the subject contains only US-ASCII characters, no encoding is done and it
	 * is used as-is. If the subject is null, the existing "Subject" field is
	 * removed.
	 * <p>
	 * 
	...
	/**
	 * Set the "Subject" header field. If the subject contains non US-ASCII
	 * characters, it will be encoded using the specified charset. If the
	 * subject contains only US-ASCII characters, no encoding is done and it is
	 * used as-is. If the subject is null, the existing "Subject" header field
	 * is removed.
	 * <p>
	 * 
	 * The application must ensure that the subject does not contain any line
	 ...
	 * creator of the message indicates that the message is complete and ready
	 * for delivery. If the date parameter is <code>null</code>, the existing
	...
	/**
	 * Returns the Date on this message was received. Returns <code>null</code>
	 * if this date cannot be obtained.
	 * <p>
	 * 
	 * Note that RFC 822 does not define a field for the received date. Hence
	 * only implementations that can provide this date need return a valid
	 * value.
	 * <p>
	 * 
	 ...
	 * represents the content-type of the content of this message. This value
	 ...
	 * <code>disposition</code> is null, any existing "Content-Disposition"
	...
	/**
	 * Set the "Content-ID" header field of this Message. If the
	 ...
	 *                if this message is obtained from a READ_ONLY folder.
	 * @exception MessagingException
	 *                An UnsupportedEncodingException may be included in the
	 *                exception chain if the charset conversion fails.
	 */
	...
	/**
	 * Get the languages specified in the "Content-Language" header field of
	 * this message. The Content-Language header is defined by RFC 1766. Returns
	 * <code>null</code> if this field is unavailable or its value is absent.
	 * <p>
	 * 
	 * This implementation uses the <code>getHeader</code> method to obtain the
	 ...
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.search.MessageIDTerm
	 * @since JavaMail 1.1
	 */
	 ...
	 * "Content-Disposition" header field of this message. If it's not
	 * available, returns the value of the "name" parameter from the
	 * "Content-Type" header field of this BodyPart. Returns <code>null</code>
	 * if both are absent.
	 * <p>
	 * 
	 ...
	 * is, it invokes <code>getDataHandler().getInputStream()</code>.
	 * 
	 ...
	 * @see com.bilgidoku.rom.epostatemel.javam.activation.DataHandler#getInputStream
	 */
	 ...
	 * to create a DataHandler object for the content. Subclasses that can
	 * provide a separate input stream for just the message content might want
	 ...
	 * <p>
	 * 
	 * The implementation provided here works approximately as follows. Note the
	 * use of the <code>getContentStream</code> method to generate the byte
	 * stream for the content. Also note that any transfer-decoding is done
	 ...
	 * unknown to the DataHandler system, an input stream is returned as the
	 * content.
	 * <p>
	 * 
	 ...
	 * Multipart or Message object and was created by parsing a stream, the
	 * object is cached and returned in subsequent calls so that modifications
	 * to the content will not be lost.
	 * 
	 * @return Object
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.Part
	 * @see com.bilgidoku.rom.epostatemel.javam.activation.DataHandler#getContent
	 * @exception IOException
	 *                this is typically thrown by the DataHandler. Refer to the
	 *                documentation for javax.activation.DataHandler for more
	 *                details.
	 ...
	 * the JavaMail implementation for this to work right. i.e., to do
	 ...
	 * new message object will be empty, i.e., it will <strong>not</strong> have
	 * a "content". These will have to be suitably filled in by the client.
	 * <p>
	 * 
	 ...
	 * message to include the contents of the "References" header (or, if
	 * missing, the "In-Reply-To" header) in this message, plus the contents of
	 * the "Message-Id" header of this message, as described in RFC 2822.
	 * 
	 * @param replyToAll
	 ...
	 * {@link com.bilgidoku.rom.epostatemel.javam.mail.Flags.Flag#ANSWERED ANSWERED} flag is set in this
	 * message.
	 * <p>
	 * 
	 * The "Subject" field is filled in with the original subject prefixed with
	 * "Re:" (unless it already starts with "Re:"). The "In-Reply-To" header is
	 * set in the new message if this message has a "Message-Id" header.
	 * 
	 * The current implementation also sets the "References" header in the new
	 * message to include the contents of the "References" header (or, if
	 * missing, the "In-Reply-To" header) in this message, plus the contents of
	 ...
	 *            reply should be sent to all recipients of this message
	 * @param setAnswered
	 *            set the ANSWERED flag in this message?
	 * @return the reply Message
	 * @exception MessagingException
	 *                for failures
	 * @since JavaMail 1.5
	...
	public Message reply(boolean replyToAll, boolean setAnswered) throws MessagingException {
		 ...
		 * encoding information. This is safe because "Re:" isn't
		 * internationalized and (generally) isn't encoded. If the entire
		 * Subject header is encoded, prefixing it with "Re: " still leaves a
		 * valid and correct encoded header.
		 */
		 ...
		 * any). If the parent has none of the "References:", "In-Reply-To:", or
	...
	/**
	 * Check addrs for any duplicates that may already be in v. Return a new
	 * array without the duplicates. Add any new addresses to v. Note that the
	 * input array may be modified.
	 */
	 ...
	 * @exception MessagingException
	 *                for other failures
	 * @see com.bilgidoku.rom.epostatemel.javam.activation.DataHandler#writeTo
	 */
	 ...
	 * @param ignoreList
	 *            the headers to not include in the output
	 * @exception IOException
	 *                if an error occurs writing to the stream or if an error is
	 *                generated by the javax.activation layer.
	 * @exception com.bilgidoku.rom.epostatemel.javam.mail.MessagingException
	 *                for other failures
	 * @see com.bilgidoku.rom.epostatemel.javam.activation.DataHandler#writeTo
	 */
	...
	public void setHeaderRom(String name, String value) {
		headers.setHeader(name, value);
	}
	 ...
	 * contains non US-ASCII characters must have been encoded as per the rules
	 * of RFC 2047.
	 * 
	 * @param name
	 *            header name
	...
	public void addHeadHeaderLine(String line) throws MessagingException {
		headers.addHeadHeaderLine(line);
	}
	 ...
	 * @exception MessagingException
	 *                for failures
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.Flags
	 */
	 ...
	 * @exception MessagingException
	 *                for failures
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.Flags.Flag
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.Flags.Flag#ANSWERED
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.Flags.Flag#DELETED
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.Flags.Flag#DRAFT
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.Flags.Flag#FLAGGED
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.Flags.Flag#RECENT
	 * @see com.bilgidoku.rom.epostatemel.javam.mail.Flags.Flag#SEEN
	 */
	 ...
	 * with the message's contents. If this message is contained in a Folder,
	 ...
	 * set), the <code>MIME-Version</code> header and the
	 * <code>Message-ID</code> header. Also, if the content of this message is a
	 * <code>MimeMultipart</code>, its <code>updateHeaders</code> method is
	 * called.
	 * <p>
	 * 
	...
	/**
	 * Create and return an InternetHeaders object that loads the headers from
	 * the given InputStream. Subclasses can override this method to return a
	 * subclass of InternetHeaders, if necessary. This implementation simply
	 * constructs and returns an InternetHeaders object.
	 * 
	 ...
	 * implementation simply constructs and returns a MimeMessage object using
