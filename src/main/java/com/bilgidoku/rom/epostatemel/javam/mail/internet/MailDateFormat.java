...
public class MailDateFormat extends SimpleDateFormat {
	...
	com.bilgidoku.rom.gunluk.Sistem.outln("------- test all timezones  ---------------");
	com.bilgidoku.rom.gunluk.Sistem.outln("Current date: " + d);
	String[] allIDs = TimeZone.getAvailableIDs();
	for (int i = 0; i < allIDs.length; i++) {
	    TimeZone tz = TimeZone.getTimeZone(allIDs[i]);
	    df.setTimeZone(tz);
	    com.bilgidoku.rom.gunluk.Sistem.outln("Date in " + tz.getID() + ":    " +
			       df.format(new Date()));
	}
	try {
        com.bilgidoku.rom.gunluk.Sistem.outln(df.parse("Sun, 21 Mar 1993 23:56:48 -0800 (PST)"));
	com.bilgidoku.rom.gunluk.Sistem.outln(df.parse("Mon, 22 Mar 1994 13:34:51 +0000"));
	com.bilgidoku.rom.gunluk.Sistem.outln(df.parse("26 Aug 76 14:29 EDT"));
	com.bilgidoku.rom.gunluk.Sistem.outln(df.parse("15 Apr 11 23:49 EST"));
	com.bilgidoku.rom.gunluk.Sistem.outln(df.parse("15 Apr 11 23:49 ABC"));
	} catch (ParseException pex) {
	    pex.Sistem.printStackTrace();
	}
	...
	com.bilgidoku.rom.gunluk.Sistem.outln("------- test all days in a month ---------------");
	...
	com.bilgidoku.rom.gunluk.Sistem.outln("Initial Date: " + cal.getTime());
	com.bilgidoku.rom.gunluk.Sistem.outln("Current Date: " + df.format(cal.getTime()));
	for (int i = 0; i < 30; i++) {
	...
	// test all months
	com.bilgidoku.rom.gunluk.Sistem.outln();
	com.bilgidoku.rom.gunluk.Sistem.outln("------- test all months in a year -----------");
	...
	// test leap years
	com.bilgidoku.rom.gunluk.Sistem.outln();
	com.bilgidoku.rom.gunluk.Sistem.outln("------- test leap years -----------");
