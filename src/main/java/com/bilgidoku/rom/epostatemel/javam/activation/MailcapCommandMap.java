...
import com.bilgidoku.rom.epostatemel.activation.registries.LogSupport;
import com.bilgidoku.rom.epostatemel.activation.registries.MailcapFile;
...
import com.bilgidoku.rom.epostatemel.javam.activation.MailcapCommandMap;
...
public class MailcapCommandMap extends CommandMap {
	...
	com.bilgidoku.rom.gunluk.Sistem.outln("Preferred Commands:");
	for (int i = 0; i < cmdInfo.length; i++)
	    com.bilgidoku.rom.gunluk.Sistem.outln("Command " + cmdInfo[i].getCommandName() + " [" +
					    cmdInfo[i].getCommandClass() + "]");
	cmdInfo = map.getAllCommands(argv[0]);
	com.bilgidoku.rom.gunluk.Sistem.outln();
	com.bilgidoku.rom.gunluk.Sistem.outln("All Commands:");
	for (int i = 0; i < cmdInfo.length; i++)
	    com.bilgidoku.rom.gunluk.Sistem.outln("Command " + cmdInfo[i].getCommandName() + " [" +
					    cmdInfo[i].getCommandClass() + "]");
	DataContentHandler dch = map.createDataContentHandler(argv[0]);
	if (dch != null)
	    com.bilgidoku.rom.gunluk.Sistem.outln("DataContentHandler " +
