...
import com.bilgidoku.rom.epostatemel.javam.mail.EncodingAware;
...
import com.bilgidoku.rom.epostatemel.javam.mail.Part;
...
public class MimeBodyPart extends BodyPart implements MimePart {
	...
	private static final boolean setDefaultTextCharset = PropUtil.getBooleanSystemProperty(
			"mail.mime.setdefaulttextcharset", true);

	private static final boolean setContentTypeFileName = PropUtil.getBooleanSystemProperty(
	...
	private static final boolean ignoreMultipartEncoding = PropUtil.getBooleanSystemProperty(
	...
	static void updateHeaders(MimePart part) throws MessagingException {
		...
		try {
			...
			/*
			 * If this is our own MimePartDataHandler, we can't update any of
			 * the headers.
			 * 
			 * If this is a MimePartDataHandler coming from another part, we
			 * need to copy over the content headers from the other part. Note
			 * that the MimePartDataHandler still refers to the original data
			 * and the original MimePart.
			 */
	...
	static void writeTo(MimePart part, OutputStream os, String[] ignoreList) throws IOException, MessagingException {
		...
		try {
			...
			if (dh instanceof MimePartDataHandler) {
				...
				/*
				 * if (part instanceof MimeBodyPart) { MimeBodyPart mbp =
				 * (MimeBodyPart)part; is = mbp.getContentStream(); } else if
				 * (part instanceof MimeMessage) { MimeMessage msg =
				 * (MimeMessage)part; is = msg.getContentStream(); }
				 */
			}
			if (is != null) {
