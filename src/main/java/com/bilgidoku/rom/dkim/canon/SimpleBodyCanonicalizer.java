...
package com.bilgidoku.rom.dkim.canon;
...
public class SimpleBodyCanonicalizer extends FilterOutputStream {
    ...
    public void write(byte[] b, int off, int len) throws IOException {
        if (len <= 0)
            return;
        if (DEEP_DEBUG)
            Sistem.outln("I:(" + lastWasCR + "|" + countCRLF + ") ["
    ...
    public void write(int b) throws IOException {
        if (DEEP_DEBUG)
            Sistem.outln("B:(" + lastWasCR + "|" + countCRLF + ") [" + "" + (char) b + "]");
        if (lastWasCR && '\n' == b) {
            lastWasCR = false;
            countCRLF++;
        } else {
    ...
    private void complete() throws IOException {
        if (DEEP_DEBUG)
            Sistem.outln("C:(" + lastWasCR + "|" + countCRLF + ")");
        if (lastWasCR) {
    ...
    private void dumpCRLF() throws IOException {
        if (DEEP_DEBUG)
            Sistem.outln("D:(" + lastWasCR + "|" + countCRLF + ")");
        if (lastWasCR) {
            out.write('\r');
            lastWasCR = false;
        }
