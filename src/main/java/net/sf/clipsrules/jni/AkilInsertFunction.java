...
public class AkilInsertFunction implements UserFunction {

	final AkilErisim env;
	final AkilExecDbOp op;

	public AkilInsertFunction(AkilErisim env) {
		this.env = env;
...
//			System.out.println("-------------------------");
	...
	private void insert(List<PrimitiveValue> arguments) throws KnownError {
		...
		for (int i = 2; i < arguments.size(); i = i + 2) {
			...
			sb.append(toSql(arguments.get(i)));
	...
	private String toSql(PrimitiveValue p) {
		if(p.isSymbol() || p.isString()) {
			return SiteUtil.singleQuoteDbText(p.getValue().toString());
		}
